...
First, the importance of that area of life to sustainability is demonstrated.
...
operate buildings and machines
...
However, everyone with a house makes decisions with sustainability implications
in their own home.
...
household_energy_by_end_use %>% 
   ...
   ggplot2::geom_text(ggplot2::aes(label = paste0(Percentage, "%")), position = ggplot2::position_stack(vjust = 0.5)) +
...
# 
# household_energy_by_end_use %>% 
#   ggplot2::ggplot(mapping = ggplot2::aes_string(x = "", y = "Percentage", fill = "End_use")) + 
#   ggplot2::geom_bar(stat = "identity", width = 1) +
#   ggplot2::coord_polar("y", start = 0) 
...
inside the dwelling compared to the outdoors,
...
Increased residential heating and cooling efficiency\index{efficiency!heating and cooling!residential}
can be obtained
...
quantified in units of hr-ft$^2$-\degree{}F/BTU.
...
Furnaces convert a fuel (typically natural gas\index{natural gas} or heating oil\index{heating oil})
...
low-flow showerheads\index{showerheads!low-flow} can be installed.
...
the Energy Star\index{Energy Star} program shows that
...
Energy and material consumption decision by household residents
...
hygiene, and
lighting, 
because these areas of everyday life drive energy consumption
