...
In this chapter, the subject is one of the kinds of collectives. 
In the next chapter, the subject is an individual.
The \emph{action} is either a law, research, or education in the examples we present
but there could be other actions.
Lastly, the \emph{object} is what the actions are applied to.
We use as example objects Chapters \ref{chap:population}-\ref{chap:land_use_urban_planning},
although, like actions, there could be many others areas where actions for sustainability 
