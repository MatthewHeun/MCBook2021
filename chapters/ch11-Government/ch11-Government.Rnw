...
which means that individuals choose to join the organization.
...
Many nonprofits consist of both a business corporate structure and voluntary organization(s).
...
corporations, 
markets, and
the natural world (for example, plants, animals, inanimate things, energy resources).
...
to set rules and regulations, manage extraction, and preserve the resource.
The commons becomes tragic when the resource is depleted by overexploitation.
Absence of a controlling authority can occur for many environmental challenges
that are global in scope
(such as international fisheries, non-localized air pollution, greenhouse-gas pollution), 
...
create a commons that can be (over) exploited for enhanced short-term financial gain.
...
Laws intersect with research when governments support research 
in a number of ways.
Through patents,
...
as when government-employed scientists develop models of the atmosphere. 
Other government research funding flows
to private entities in the form of grants. 
For example, sustainability research funded by the 
...
These examples are self-consciously nonexhaustive. 
...
Of the four $PARC$ terms, resource intensity of the economy~\Rparen{} 
...
One way to reduce the impact intensity of resources~\Xparen{}
...
That said, governments are made up of people.
