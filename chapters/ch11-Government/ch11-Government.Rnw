...
Subjects can be collectives or individuals who perform the action.
There are many possible actions but this book considers four examples:
choices, laws, research, and education.
We focus on eight objects or areas of application corresponding to 
the topics of Chapters~\ref{chap:population}--\ref{chap:land_use_urban_planning}. 
...
digging deeper into the ideas and concepts mentioned herein
to find the actions that fit their circumstances and outlook.
...
It might be a single, simple choice or lifestyle-related (driving vs.\ walking).
