...
This is the first of two chapters to discuss actions to encourage
society and individuals to live more sustainably.
This chapter (\ref{chap:government}) explores collective actions and 
the next (\ref{chap:personal_action}) explores personal actions.
Collective actions usually involve changing the rules of the game 
...
that encourage unsustainable practices or consumption.
...
(and sometimes define and sometimes effect)
...
In some cases, laws and regs define what citizens and corporations cannot do.  
...
The goal of this this chapter is to provide references to 
ways to actively promote sustainability via collective action.
Readers are encouraged to explore the suggested actions that fit their
local situation, 
always keeping the global perspective provided 
by Chapters~\ref{chap:introduction}--\ref{chap:carbon_intensity} in mind.
...
Equation~\ref{eq:IPARC} shows that rising affluence ($A$)
drives environmental impacts ($I$). 
Ironically, the purpose of many government actions 
is to produce affluence, 
...
Equation~\ref{eq:IPARC} shows that rising resource intensity of the economy ($R$)
drives environmental impacts ($I$).
To reduce environmental impacts, 
economies need to use fewer resources and emit less pollution per dollar of GDP.
Figure~\ref{fig:EnergyIntensityGraph-Nordhaus} shows that the 
...
will reduce energy required to generate a dollar of GDP
at the location of the energy use.
However, the rebound effect and energy efficiency backfire can reduce the effectiveness
of energy efficiency measures
to reduce total energy consumption across the economy. 
...
can reduce (or in the case of backfire) eliminate the local energy consumption 
cause by the energy efficiency measure. 
To reduce rebound or backfire, 
businesses can develop green revolving funds,
...
from which businesses and individuals perform energy efficiency upgrades
...
thereby extending the life of reserves and increasing the liklihood that a 
...
manufacturing.
That said, even a service economy needs goods, 
so the move to a service-based economy will entail some production.
And the move to a service-based economy carries risks that production is simply off-shored,
not eliminated altogether,
thereby increasing the resource intensity of the producing economy.
...
\begin{figure}
\centering
\includegraphics[width=\figwidth]{chapters/ch11-Government/photos/Energy_Star.jpeg}
\caption{Energy star label on a furnace.}
...
(Figure~\ref{fig:energy_star} shows an energy star label on a furnace.)
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.48\textwidth}
         \centering
           \includegraphics[width=\linewidth]{chapters/ch11-Government/photos/fill_photo_portrait.jpeg}
         \caption{Bus depot.}
         \label{fig:bus_depot}
     \end{subfigure}
...
This Section (?) discusses collective actions that can be taken at the systematic
level to improve the sustainability of food and agriculture.
...
can be used to disseminate to farmers knowledge about best practices in farming ecologically.
...
Labeling agreements should be reached to standardize terminology and reporting on
food labels: terms like ``organic,'' ``natural,'' ``free range,'' and ``grass fed.''
Lastly, farm subsidies should be eliminated or reformed to stop wasting energy on
biofuels with a very low EROI (see Table~\ref{tab:eroi}). Figure~\ref{fig:E85} shows
a commercially available biofuel blend with low EROI.
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.48\textwidth}
         \centering
           \includegraphics[width=\linewidth]{chapters/ch11-Government/photos/fill_photo_portrait.jpeg}
         \caption{Suburban grocery store.}
