...
that is, \emph{making and selling products and services}.
...
but they provide a \emph{framework}
...
Characteristic of the ``choice'' action is that it has a \emph{direct impact} on sustainability, and
it involves choosing a sustainable option from a menu of \emph{existing} activities or products.
...
In some cases, laws and regulations define what \emph{cannot} be done.  
For example, some types of pollution, such as dumping oil in waterways, are prohibited.
In other cases, laws and regulations define what \emph{must} be done.
...
to define \emph{how} things must be done.
...
% Collective actions often require the cooperation of several groups to be successful.
% This chapter considers collective actions that might be taken by
% governments, corporations, and organizations.
% 
% 
% %++++++++++++++++++++++++++++++
...
% %++++++++++++++++++++++++++++++
% 
% The second type of collective is business corporations. 
% Examples include Volkswagen, Siemens Gamesa Renewable Energy,
% the Saudi Arabian Oil Company, Maersk, and Best Buy.
...
% %++++++++++++++++++++++++++++++
% 
% The third type of collective is an organization.
% Example organizations include the American Society of Heating, Refrigerating and 
% Air-Conditioning Engineers (ASHRAE),  the 3$^{\text{rd}}$ Generation Partnership Project (3GPP),
% local churches, the Internet Society, political
% parties, environmental organizations, and community and civic groups.
...
% Lastly, the Internet Society (mentioned above)
% is an organization that sets policy for high-level internet activities; 
% it is an organization that is also quasi-governmental in nature.
% 
% %^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
% \begin{mcframe}[0.90\textwidth](0.85\textwidth)
% %^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
% Although these four actions apply to all kinds of policy problems, 
% we focus on applications for sustainability.
...
% I commented the next sentence, because it is a repeat of two sentences above.
% A second important caveat is that we are considering only actions 
% \emph{for sustainability}.
% 
% I commented the next sentence, because it doesn't seem to me that there is anything
% special about the sentences and sustainability.
% The sentence structure *could* be applied to non-sustainability areas.
...
% Each section below offers several answers to that questions, 
% each organized as a sustainability sentence. 
% These examples are nonexhaustive. 
...
In the subsections below, sustainability sentences are 
...
in~(a) it is \emph{efficiency},
in~(b) it's \emph{cost}, and
in~(c) it's the \emph{amount} of energy used.
...
\item Carbon emissions from combustion of fossil fuels are example of negative externalities. 
      ...
      (a)~a tax on fossil fuels \emph{or} 
