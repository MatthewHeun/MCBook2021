...
In contrast, we will define %, partly because it makes Equation \ref{eq:IPARX} a "pronounceable" acronym,
the term ``affluence'' to mean the \emph{rate} a person 
uses up goods and services, that is, the ``stuff'' bought \emph{per year}.

Our economic activity may mean literal consumption, as in the case with food, 
or it could be consuming a service, such as education or getting a haircut. 
...
%
% However, each individual has a different set of preferences about what will make them
% happy.
...
% from Grawe:
Moreover, each individual has a different preferences about what will make them 
happy and different abilities (to produce goods and services). 
The existence of these differences guarantees potential gains from trade. 
Both parties in a voluntary exchange increase their own happiness by trading. 
In other words, trade makes people better off.
...
affluence. 
% To the degree that the proposal involves something costing more,
...
% from Grawe:
(When we speak of affluence we mean real affluence adjusted for changes in the 
price level over time.) %boy, this doesn't read well.
...
In any case, a different measure of affluence \mft{A}, would require a corresponding 
adjustment to (the measure of) resource intensity \mft{R}. 
The end result leaves us in the same difficult spot: Consuming more per person 
creates greater impact if the resource intensity of affluence 
...
affluence and maintaining continual growth for GDP -- even though 
perpetual growth on finite resources is ultimately unsustainable
...
for the remainder of the chapter we will use the term more colloquially to include 
everything we use in personal consumption, government spending, and investment,
that is, global GDP.
Individuals and corporations may save money for purchases in the future, but the
economy as a whole balances production and (a more generic) consumption as 
closely as possible.
\[ \label{eq:PCbalance}
...
Resources (\:\!\mft{R}) are the feed the economy needs to live and grow.
The impact intensity of resources, \mft{X}, represents the wastes that the economy excretes.
...
Growing affluence is, by definition, an increase in standard of living. 
...
Output growth means that each year the mine must produce more 
than it did the previous year.
A standard of living increase means consuming \emph{more} this year than last year.
To provide for increased consumption, there must be increased production.
...
However, if you look close enough, almost all of these ``new goods'' stories end
up being about some form of efficiency.
...
Figures~\ref{fig:per_capita_material_use} and~\ref{EnergyPerPersonGraph},
...
(that is, exponential growth\index{exponential growth}; see \ref{project:alt_exp}).
As has been mentioned a few times already, exponential growth\index{exponential growth} is inherently
unsustainable. 
...
Another alternative is a \emph{circular} economy\index{economy!circular}, which is one that exists only
