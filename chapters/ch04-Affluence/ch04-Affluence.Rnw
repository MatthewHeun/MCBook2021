...
such as the work of stay-at-home mothers.
Also, GDP counts all consumption, even when it is socially undesirable.

Many new metrics (Section~\ref{sec:indices}) have been proposed as replacements for
or improvements to GDP, such as
green GDP, the index of sustainable economic welfare, and the
...
trends through time, which are usually \emph{qualitatively} 
consistent with trends would be observed using other indices.
...
(discussed in the next two chapters) remain unchanged.
...
Therefore, for world GDP, \mft{nx}~=~0.
(We don't import from, or export to, space \ldots yet.)
...
% JVA: remove the next sentence? ---MKH
...
Either
(a)~by consuming \emph{more} inputs 
or
(b)~by using existing inputs more \emph{efficiently} (meaning that \mft{R} goes down).
