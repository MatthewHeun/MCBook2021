...
the burns gasoline or is electric doesn’t matter to the consumer. 
...
sixth donuts increase happiness less than the first, second, and third donuts. 
After a donut, we may want a cup of coffee instead of a second donut, then a 
slice of bacon, and then to read the news, and then \ldots . 
The point is there is always a next ``thing,'' which need not necessarily 
be a physical object. 

Having choice enhances consumer well being. 
A choice between a muffin and a donut is better than if only donuts are 
on the menu. 
(Again, there is diminishing marginal utility for choice as well; 
partly this is because of human cognitive limits.)

The value functions of different individuals may be different and so free trade 
allows each party to maximize their own value function. 
The opportunity cost is the value (to them) of what they give up, be it money or 
a good or service. 
...
(Note that an abstract desire to have lower impacts is not the same as wanting to be poor.)
...
of innovation in improving standards of living. 
...
Frankly, GDP is what we have and what ``everyone'' uses. 
GDP is the standard. 
...
\label{eq:GDP}
  GDP = C+G+I+NX \; .
\end{equation}
$C$ is consumption and represents purchases made by individuals. 
Consumption makes up about two thirds of GDP for developed economies and is
generally divided into three categories: durable goods, nondurable goods, and services. 
...
Examples include appliances, automobiles, but don’t include fabrics (like clothes 
or carpet) even if those things happen to last more than three years. 
...
goods spending because consumers can choose to postpone spending on durable goods 
until their circumstances are better. 

$G$ is government spending, which is divided between government consumption 
(such as, equipment and wages paid to government workers) and net investment, for 
example, infrastructure improvements.

The symbol $I$ stands for investment, which is \emph{not} money put into a bank account. 
Investment is goods that are purchased for use in the future. 
...
Therefore, for world GDP, $NX=0$. We don't import from or export to space, yet.
...
Another way of looking at Equation \ref{eq:IPARC} is that the impacts produced
are really a result of consumption, that is the affluence term.
The $R \times C$ terms are just a mechanism or feed for the economy.
To put it another way, if there weren't consumption occurring, there wouldn't be
any $R \times C$.
To put it a third way, resources $R$ feed the economy and the resulting impacts
$C$ are the wastes that the economy excretes.

\section{Economic growth}
\label{sec:EconGrowth}
Although production and consumption are in balance, both may grow together and,
indeed it is desirable that they do so.
...
Each year the mine must produce more than it did the previous year.

The economy can grow by consuming more resources, such as energy, (and, therefore, 
creating more wastes) which is called \emph{extensive} growth, or by using inputs 
(resources) more efficiently, which is called \emph{intensive} growth.
The factors that determine how much goods and services an economy can produce
include labor, capital, energy, and materials.
The productivity of workers (labor productivity) is influenced by human capital,
which accounts for the education and skill of workers, the number of workers
in the population, the hours they work, and the faction of the population that is
working (see Section \ref{sec:dtt}). 
A major source of economic growth is innovation.
The majority of the goods and services we consume today did not exist in 1870.

Lastly, the productivity of an economy is influenced by its \emph{level} of 
capital investment.
Capital is both buildings and equipment, machines, and tools.
Because capital depreciates, continual investment (part of $I$ in Equation \ref{eq:GDP})
...
maintain it.
At some point there is a maximum level of effective capital.
Beyond that point, the capital is more expensive to maintain than it contributes 
to the economy.
Where the point is depends on the level of technology and the price of borrowing
...
level of output. At this point, an economy can grow only through access to new 
resources (such as energy) or innovation (a change in the level of technology).

\section{Opportunities for reduced impact}
...
environment can provide, that is, it exceeds the ecosystem's ability to provision,
and/or the wastes generated exceed the ecosystem's ability to absorb, that is,
the ecosystem's regulating and support services pass a tipping point.
...
The costs are real and there has been no real political will or action to make 
...
The above proposals address \emph{rates} but they don't address \emph{levels} 
and it is the level of economic activity that matters for sustainability -- as 
exemplified in Figure \ref{fig:FootprintHDIGraph}.
...
\item why do we have (global/local) poverty beside (global/local) plenty?
...
\item GDP, because it represents consumption, is only a partial indication of wealth. 
      ...
      inequality different? Which do you think it worse from a social perspective
...
\item One strategy to make electricity grids more efficient is to employ load shedding.
      ...
      to give up (some) control (choice) of your electricity consumption?
...
\item Stopping economic growth is one way to limit environmental impacts. 
      Developed economies do not wish to lower their standard of living and 
      developing economies want their opportunity to enjoy higher standards of 
      living. Hence the idea of ``sustainable development.'' Explain how developed
      economies telling less-developed economies not to impact the environment
      feels like neo-imperialism. On the other hand, developed countries have
      lots of experience with what not to do. What are some communication strategies
      that are helpful for cross-cultural projects?
      
\item It has been observed that goods can move relatively freely across international
      borders but labor can't. What are the consequences of leaving behind the 
      environmental and social impacts of manufacturing when goods travel globally?
      (Divorcing the product from its manufacturing impacts.) What are the economic
      effects?

\item ``It's hard to fight economics.'' What does this statement imply for
      environmental and social sustainability sustainability?      
...
\item Sometimes you'll hear (well meaning) people blame corporations for wanting
      ...
      wasteful. All of us should be as efficient as possible. To what extent is 
      this true? In what ways? Are there limits and what are they? (Other approaches
      to this question are presented in Chapter \ref{chap:government}.)
...
\item Efforts to develop in ways that are more environmentally friendly are often
      ...
      to provide assistance to developing economies do development ``right.''
      Many times projects are unsuccessful. Local corruption might waste funds
      \emph{or} international aid agencies might attempt to maintain control to 
      prevent corruption but they don't know the local context and so ultimately
      end up being ineffective. Research examples of successful development projects.
...
\item What are ways the governments try to control the rate of economic growth?
