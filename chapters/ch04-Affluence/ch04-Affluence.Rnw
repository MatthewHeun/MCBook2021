...
In contrast, affluence ($A$) is the \emph{rate} that the economy
uses up goods and services, that is, the amount of ``stuff'' bought \emph{per year}.
...
Average affluence ($A$) for a country or the world
...
Effectively, GDP is the standard.
...
In any case, a different measure of affluence~($A$), would require a corresponding
adjustment to (the measure of) resource intensity of economic activity~($R$).
Regardless of the measure of affluence, we land in the same difficult spot:
greater affluence~($A$) creates greater environmental impact~($I$)
if the resource intensity of the economy~($R$) and the impact intensity of resources~($X$)
(discussed in the next chapter) is unchanged.
...
% <<AffluenceGraph-Maddison, fig.cap = "Annual world GDP per capita, affluence ($A$) in Equation \ref{eq:IPARX}. Data for 1990--2020 from the World Bank~\\cite{World-Bank:2021un}. Data for 1800--1989 inferred from GDP per capita growth rates from the Maddison Project Database 2020 ~\\cite{Bolt:2020tv}.">>=
...
% <<AffluenceGraph-DeLong, fig.cap = "Annual world GDP per capita, affluence ($A$) in Equation \ref{eq:IPARX}. Data for 1990--2020 from the World Bank~\\cite{World-Bank:2021un}. Data for 1800--1989 inferred from GDP per capita growth rates from De Long.">>=
...
<<AffluenceGraph-Nordhaus, fig.cap = paste0("Annual world GDP per capita, affluence ($A$) in Equation~\\ref{eq:IPARX}. ",  fit_sentence(gdppc_fit), " \\emph{Sources}: data for 1800--1989 inferred from GDP per capita growth rates from \\citet{Nordhaus:1997vi} via \\citet{DeLong:1997ui}; data for 1990--2020 from the \\citet{World-Bank:2021un}.")>>=
...
GDP data are collected nationally and aggregated internationally, 
so there is often skepticism of data provided by corrupt and/or authoritarian
...
Government spending~($g$) is comprised of government consumption
...
Net exports~($nx$) are the \emph{net} difference between the value of exports and the value of imports.
Thus, $nx$ could be a negative number for a country that imports more than it exports.
...
Although economists use consumption narrowly as described in Equation~\ref{eq:GDP},
we use the term colloquially to mean the sum of personal consumption, 
government spending, and investment.
That is, we use the term ``consumption'' to mean global GDP, in part to remind ourselves
...
Thus, the economy is society's metabolism.

\end{mcframe}

One way of looking at Equation~\ref{eq:IPARX} is that impacts ($I$)
are the \emph{result} of economic activity.
That is, environmental impacts are the result of society's metabolism.
Total consumption (that is, world GDP) is $P \times A$ in Equation~\ref{eq:IPARX}.
...
Indeed, growth in GDP per capita~($A$) is the explicit goal of many economic policies.
There are two ways the economy can increase GDP.
Either (a)~the economy consumes \emph{more} inputs or
(b)~it uses existing inputs more \emph{efficiently}.
...
affluence~($A$) because more workers are needed to gather, process, and use the 
additional resources.
That leaves only efficiency as a means of boosting affluence.
From a sustainability point of view, we should want to everything to be used
...
work, and the faction of the population that is working;
see Section~\ref{sec:dtt}.
More workers will increase economic output but will not increase GDP \emph{per person}.
A greater fraction of the population working, such as with the demographic dividend, 
or workers working longer hours will tend to increase $A$ (at least until the 
...
Much of the improvement in our standard of living comes from these innovations.
...
% ****
Managing the ``rebound effect,'' which occurs when improvements in efficiency result in
greater total consumption, is a major sustainability challenge;
...
If affluence $A$ is a cause of impacts $I$, then reducing $A$ is one way to reduce
...
that is, sometimes reducing $A$ \emph{increases} (at least some parts of) $I$.
...
What if there were a way or way(s) to reduce the size of the economy that did not
...
World economies are managed for continued growth. 
(See P1.\ref{project:alt_exp} for the relationship between continual growth
and exponential growth.)
As mentioned previously, continual growth is inherently
...
exceed exceed the ecosystem's ability to absorb wastes.
Eventually, economic growth will exceed a tipping point where the
ecosystem is no longer able to provide its services to the economy, as 
...
A steady-state economy would experience no change in the standard of living from 
one generation to the next,
as was (nearly) the case in the ancient and medieval worlds.
...
those economies will not grow to be \emph{more} unsustainable each year. 
...
on economic activity and tries respect planetary boundaries by equalizing income 
...
Second, there are not successful templates for degrowth to the outside of the doughnut
...
(4)~The esteem or family, friends, and others?
...
% reducing affluence ($A$)---doing so will make people less well off. 
% ****
...
This chapter's end-of-chapter questions further explore of the thorny issues 
...
% Is it meant to be a transition from macro and average affluence ($A$)?
