...
affluence~\Aparen{}, and the role of human economic activity in sustainability challenges.
...
The word ``wealth'' can refer to notions of happiness, prosperity, well being, and welfare,
but mostly wealth refers to the \emph{amount} of goods, possessions, money,
property, or ``stuff'' owned by a person.
In other words, wealth refers to a \emph{level} of material possessions or resources.
In contrast, affluence~\Aparen{} is the \emph{rate} that the economy
uses goods and services, that is, the amount of ``stuff'' bought \emph{per year}.

Economic activity may mean literally consumption, as with food,
or it could mean using a service, such as education or getting a haircut.
Sometimes, what is ``consumed'' is ancillary to what the consumer actually desires.
When travelling from point A to point B, 
most travelers don't care whether their car runs on electricity or gasoline.
The energy consumption is only the means to the end.
In this example, transport is known as an ``energy service,''
the purpose of travel.

Average affluence~\Aparen{} for a country or the world
is \emph{measured} by gross domestic product (GDP) per capita.
There are valid critiques as to why per capita GDP is not the \emph{best possible}
...
GDP doesn't include the value of non-market transactions, 
such as the work of stay-at-home moms.
Also, GDP counts all consumption, even when that consumption is socially undesirable.

Many new metrics (Section~\ref{sec:indices}) have been proposed as replacements
...
At the moment, we have the most complete and accurate data for GDP;
consequently, it is the affluence indicator ``everyone'' uses.
Effectively, GDP is the standard way to measure economic activity.
(Because GDP data are collected nationally but aggregated internationally, 
there is often skepticism of data provided by corrupt and/or authoritarian
...
trends through time, which are usually \emph{qualitatively} consistent with trends would be observed
using other indices, bearing in mind the limitations of GDP.
...
if the resource intensity of the economy~\Rparen{} and the impact intensity of resources~\Xparen{}
(discussed in the next chapter) remain unchanged.
...
%   ggplot2::labs(x = NULL, 
%                 y = expression("Affluence (A) [2017 Intl. k$/" * person%.%year * "]")) +
%   
%   MKHthemes::xy_theme()
% @
...
<<AffluenceGraph-Nordhaus, fig.cap = paste0("Annual world GDP per capita, affluence~\\Aparen{} in Equation~\\ref{eq:IPARX}. ",  fit_sentence(gdppc_fit), " \\emph{Sources}: data for 1800--1989 inferred from GDP per capita growth rates from \\citet{Nordhaus:1997vi} via \\citet{DeLong:1997ui}; data for 1990--2020 from the \\citet{World-Bank:2021un}.")>>=
...
<<GDPGraph-Nordhaus, fig.cap=paste0("World annual GDP calculated by multiplying world population in Figure~\\ref{fig:PopulationGraph} and GDP per capita in Figure~\\ref{fig:AffluenceGraph-Nordhaus}.", fit_sentence(gdp_fit))>>=
# Gather world population data
world_pop <- readxl::read_excel(file.path("chapters", "ch04-Affluence", "datasets", "WorldPopulation.xlsx"),
                                sheet = "world_pop") %>%
  dplyr::select(-Source)
...
Note that many national and international economic policies are intended to
continually increase affluence and/or maintain growth in GDP.
Figures~\ref{fig:AffluenceGraph-Nordhaus} and~\ref{fig:GDPGraph-Nordhaus} show
that these choices have been effective.
Both world GDP \emph{and} affluence \Aparen{} have been growing exponentially 
since 1950 at rates of 
\Sexpr{gdp_growth_rate}~\%/year and 
\Sexpr{affluence_growth_rate}~\%/year, 
respectively.
...
Consumption~(\mft{c}) is purchases made by individuals.
Typically, consumption is about two thirds of GDP for developed economies and is
usually divided into three subcategories: services, durable goods, and nondurable goods.
Services represent nontangible things, such as haircuts, legal advice, education,
and concert tickets.
...
goods spending,
because consumers can usually choose to postpone spending on durable goods
...
Government spending~(\mft{g}) is government consumption
(such as equipment and wages paid to government workers) plus 
government investment spending, for
example, spending on infrastructure improvements such as roads or dams.

%..............................
\paragraph{Investment}
%..............................

Investment~(\mft{i}) is \emph{not} money put into a bank account.
Investment is goods that are purchased now for use in the future
by individuals or firms (not governments).
...
Net exports~(\mft{nx}) are the \emph{net} difference between the value of exports and the value of imports.
Thus~(\mft{nx}) could be a negative number for a country that imports more than it exports.
...
%++++++++++++++++++++++++++++++
\subsection{Production/affluence balance}
\label{sec:PCbalance}
%++++++++++++++++++++++++++++++

Although economists use consumption~(\mft{c}) narrowly as described above,
we use the word ``consumption'' colloquially to mean the sum of personal consumption, 
government spending, and investment.
That is, we use ``consumption'' to mean GDP, in part to remind ourselves
...
economy as a whole balances production and (this more generic form of) consumption as 
closely as possible, such that

\begin{equation} \label{eq:PCbalance}
  \mathrm{Production} = \mathrm{Consumption} \; .
\end{equation}
%
Money that is deposited in a bank is lent out to borrowers so that they can make
purchases.
...
why would you pay the costs of production if there were no prospects of a sale?
...
% supply and demand.
%
Therefore, we use the terms ``production'' and ``consumption'' interchangeably.
...
Since the economy only consumes what the economy produces and,
likewise, only things that will be consumed are produced, we are ready to introduce
the metaphor of the economic metabolism.
All plant and animal metabolisms consume resources
(energy and materials) 
and produce wastes in the process of doing useful things.
Similarly, economies take in resources and produce wastes.

%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
\begin{mcframe}[0.90\textwidth](0.85\textwidth)
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Thus, we say the economy is society's metabolism.

\end{mcframe}

One way of looking at Equation~\ref{eq:IPARX} is that impacts~\Iparen{}
are the \emph{result} of economic activity.
That is, environmental impacts are the result of society's metabolism.
Total consumption (that is, world GDP) is $\mft{P}\times\mft{A}$ in Equation~\ref{eq:IPARX}.
Resources~\Rparen{} are the food the economy needs to live and grow.
The impact intensity of resources~\Xparen{} 
represents the wastes that the economy excretes.
...
Although production and consumption are in balance, both may change together.
Indeed, growth in GDP per capita~\Aparen{} is the explicit goal of many economic policies.
There are two ways the economy can increase GDP.
Either (a)~the economy consumes \emph{more} inputs or
(b)~it uses existing inputs more \emph{efficiently} (meaning that \mft{R} goes down).
...
affluence~\Aparen{} because more workers are needed to gather, process, and use the 
additional resources.
That leaves only efficiency as a means of boosting affluence.
From a sustainability point of view, we should want everything to be used
as efficiently as possible; inefficiency of any kind is not desirable.

\paragraph{Labor} The productivity of labor depends on the education and skill 
...
but will not increase GDP \emph{per person}.
On the other hand, a greater fraction of the population working, 
such as with the demographic dividend, 
or workers working longer hours will tend to increase \mft{A} (at least until the 
population pyramid stabilizes, in the case of the demographic dividend).

\paragraph{Capital} Capital is buildings, equipment, machines, and tools---\emph{things}
...
Higher levels of capital require larger yearly investments to maintain.
At some point, there is a \emph{maximum} level of effective capital.
Beyond that maximum, additional capital is more expensive to maintain than it contributes 
to the economy.
% The maximum effective level depends on technology and the price of borrowing
% money (that is, interest) for making capital investments.
% Conceptually, 
At the effective maximum level of effective capital, labor will have a maximum
level of output because workers are fully ``tooled up.''

\paragraph{Innovation} Innovation is the major source of economic growth.
...
\emph{also} end up being about some form of efficiency.
The endless quest to meet our needs more efficiently allows us to instead spend
our budget on \emph{additional} consumption.
Efficiency improvements allow us to get more GDP for the same level of resource consumption.
However, as we will see in Chapter~\ref{chap:resource_intensity}, particularly in 
Figures~\ref{fig:per_capita_material_use} and~\ref{fig:EnergyPerPersonGraph},
economic growth results in \emph{more} consumption of resources.
The greater use of resources is caused by affluence growth, 
driven, in part, by improvements in material (Figure~\ref{fig:per_gdp_material_use})
and energy (Figure~\ref{fig:EnergyIntensityGraph-Nordhaus}) efficiency 
(that is, ``innovation,'' as defined above).
% ****
% MKH to check, especially for efficiency vs. efficacy (see also the paragraphs above) -- JVA
% ****
Managing the ``rebound effect,'' which is a result of efficiency improvements,
is a major sustainability challenge; see Section~\ref{sec:rebound_effect}.
% 
% *****
% Would it be possible to also talk about economic contraction here? 
% Would be nice to discuss HOW economies contract and the pain it brings.
% But can also reference Figure~\ref{fig:CarbonEmissionsGraph}
% to reinforce the point that recessions reduce environmental impact.
% Talking about contraction then opens the door to discuss economic level.
% We have a choice about the economic level at which we operate.
% ---MKH 
% 
% Check the new paragraph below --- JVA
% *****

\paragraph{Recession}
Economies can grow (particularly when we try to make them grow) but they can also
shrink.
A shrinking economy is said to be in recession.
A full discussion of the mechanisms of recession is beyond the scope of this book
but the effect of recessions can be seen in Figure~\ref{fig:CarbonEmissionsGraph}.
If affluence \mft{A} is a cause of impacts \mft{I}, then reducing \mft{A} is one way to reduce
$I$, as Figure~\ref{fig:CarbonEmissionsGraph} shows for CO$_2$ emissions.
However, recessions are unwelcome for many very good reasons. 
No country has yet intentionally tried to shrink its economy.

Moreover, the environmental effects of recessions are not always positive,
that is, sometimes reducing \mft{A} \emph{increases} (at least some parts of) \mft{I}.
For instance, a company going out of business might cause 
environmental contamination because there no longer exists a responsible steward for 
potentially hazardous materials or equipment.
The U.S.\ has hundreds of thousands of leaking underground gasoline storage tanks
from gas stations that have gone out of business~\cite{brownfields}.
...
generate the negative effects of a recession?
The next section examines some ideas that have been proposed as alternatives
to continued economic growth.
(Note that we \emph{examine} these ideas without necessarily \emph{endorsing} them.)
...
% (See P1.\ref{project:alt_exp} for the relationship between continual growth
...
exceed the ecosystem's ability to absorb wastes.
Eventually, economic growth will push the ecosystem past a tipping point where the
ecosystem is no longer able to provide ecosystem services, as 
discussed in Chapter~\ref{chap:planetary_boundaries}.

In response, there are calls for transitioning world economies 
to new operating regimes, different from growth. 
There are proposals for 
zero-growth (or steady-state), 
degrowth (or shrinking),  
circular, and 
doughnut
economies. 
...
A steady-state economy\index{economy!steady-state} is one that neither grows nor shrinks.
Its size (measured in \$/year) stays the same through time.
With a stable population, a steady-state economy would experience no change 
in the standard of living \Aparen{} from one generation to the next.
If population goes up or down, 
affluence will go down or up, 
because the size of the economy is fixed.

One criticism of a steady-state economy is that (some) people could be left
poor and destitute. 
The static nature of a steady-state economy might strengthen or rigidify already
unacceptable inequalities within our economic system.
(At least, that is the fear.)

Further, a steady-state economy is not necessarily sustainable.
The sustainability of an economy is not determined by its growth rate
(usually measured in \%/year), 
the quantity held constant in a steady state economy. 
Rather, the sustainability of an economy is determined by its \emph{level} or \emph{size}
(usually measured in \$/year) relative to planetary boundaries. 
If the steady-state level of the economy 
is above a sustainable threshold
(if planetary limits have been exceeded), 
the economy is unsustainable, regardless of whether it is growing (positive \%/year), 
shrinking (negative \%/year), or 
steady state (0~\%/year). 
Indeed, Figure~\ref{fig:FootprintHDIGraph} suggests that
many economies are \emph{already} too large to be considered sustainable.

Figure~\ref{fig:FootprintHDIGraph} further highlights the fact that 
many economies are \emph{both} already too large \emph{and} still growing.
...
those economies will not grow to become \emph{even more} unsustainable each year. 

%..............................
\paragraph{Degrowth} \index{degrowth}
%..............................

In contrast to the steady-state economy, 
degrowth advocates embrace the fact that many economies need to become smaller
to be sustainable.
Degrowth advocates focus on sufficiency and human wellbeing
instead of GDP growth.

There are substantial social and political pressures against degrowth.
And for good reason!
...
Furthermore, there are costs and difficulties associated with \emph{transitioning}\index{transition!economic}
...
crisis.
On the other hand, perhaps if population were to fall faster than the rate of 
economic degrowth, it would be possible to have rising affluence along with
lower environmental impacts.

%..............................
\paragraph{Circular economy} \index{economy!circular}
%..............................

Another alternative to today's growth economies
is a \emph{circular} economy\index{economy!circular}, 
which (a)~consumes only recycled (and, perhaps, renewable) materials and energy
and (b)~emits no waste. 
A circular economy is impossible to achieve completely.
Materials cannot be 100\% recycled and, for many materials, the recycling process
degrades material properties. 

It is unclear whether a circular economy is environmentally sustainable.
The biggest flaw in the idea of a circular economy is that it requires continual energy input,
in part because recycling of materials requires energy;
see Section~\ref{sec:recycling}.
Currently, the supply of renewable energy is insufficient to run a circular
economy.
To make a circular economy work, either renewable energy would have to be scaled
up dramatically or the entire economy would have to be scaled down to match 
the current supply of renewable energy.

That said, the circular economy provides a helpful model to strive toward.
A circular (or partly circular) economy would bring us closer to sustainability.

%..............................
\paragraph{Doughnut economy} \index{economy!doughnut}
%..............................

A doughnut economy keeps the size of economic activity
within planetary boundaries (represented by an outer circle) while simultaneously
lifting people out of poverty (represented by an inner circle).
The space between the two circles is shaped like a doughnut and represents the ``safe operating 
space'' for humanity. 
The outer circle is a form of steady-state economy (perhaps changing as technology
changes).
Since a steady-state economy could leave (some) people poor and destitute, 
the doughnut economy attempts to correct this flaw by imposing both a ``floor'' 
and a ``ceiling'' on economic activity and tries to respect planetary boundaries 
by equalizing income disparity between rich and poor.

Although a doughnut economy sounds good in theory, 
there would be many conceptual and
operational difficulties in implementation. 
To one degree or another these difficulties are shared with the steady-state,
degrowth, and circular economy models.
First, tying economic level(s) to particular planetary boundaries might not be
straightforward, particularly since we don't know with certainty exactly what those 
boundaries are, in some cases.
Probably, a better approach is to address, for example, CO$_2$ emissions directly.
Second, there are no successful examples of either degrowth to move within the doughnut
or redistribution of affluence within the doughnut.
Relatedly, who sets the ``floor'' and the time period for transition?
Third, the doughnut model implicitly sets up a ``people-versus-planet'' dichotomy
and then attempts to maximize the ``people'' side 
(to the extent permitted by the ``planet'' side).
Might it be possible to have flourishing society \emph{and} a flourishing planet?
...
The above hierarchy of needs~\cite{Maslow1954}, 
can frame the question of ``what do we really want?''

The recent work of Lina Brand-Correa, Julia Steinberger, and others
frame the question of ``what do we want from life'' in energy terms~\cite{Brand-Correa:2017aa}.
They search for ways to decouple satisfaction of human needs from 
energy services, energy consumption, and CO$_2$ emissions.
...
Conversely, although happiness \emph{can} be found in the midst of financial hardship or poverty, 
most people don't wish to go through financial trouble to achieve happiness.
Any reduction in affluence % or choice 
runs counter to what people want (or say they want).

When we look at sustainability through the economic lens, Figure \ref{fig:FootprintHDIGraph}
suggests the only way to live sustainably is poverty. 
Indeed, many proposals for becoming more sustainable are effectively reductions of
affluence.
However, poverty is a hard sell on an individual level; no one wants it for themselves.
(The abstract desire to have lower impacts is not the same as wanting to be poor.)
Lower levels of affluence are associated with lower HDI, as 
Figure~\ref{fig:FootprintHDIGraph} shows (because income is a factor in HDI).
Globally, social sustainability is often predicated on \emph{increasing} affluence.

Some people \emph{have} chosen (relative) poverty by trying to escape the economic
system; see P4.\ref{project:off-grid} for a few examples.
Often, for these people, independence or being ``off grid'' is the main attraction.
However, even in the most extreme cases, it has proven impossible to completely sever an 
economic relationship to the rest of humanity.
There is always ends some participation in the broader economy,
so there is no way to escape the sustainability implications of economic activity.
...
This chapter defines affluence~\Aparen{} in the IPARX equation and looked at 
historical data of \mft{A} over time. 
Growth in affluence can be viewed as the source (or \emph{a} major source)
of our many sustainability challenges.
This chapter's discussion questions further explore the thorny relationship  
between affluence~\Aparen{} and impacts~\Iparen{}.
On the other hand, increasing affluence is desirable. 
Indeed, it is the goal of many aspects of social sustainability.
However, growth in affluence puts the burden of lowering impacts~\Iparen{} on the
other terms on the right-hand side of the IPARX equation:
population~\Pparen{}, the resource intensity of economic activity~\Rparen, 
...
The next two chapters examine sustainability related to \mft{R} and \mft{X} terms.
...
Robert J. Gordon, \emph{The rise and fall of American growth: The U.S. standard of
living since the Civil War}, Princeton, NJ: Princeton University Press, 2016. \\
...
\noindent
N. Gregory Mankiw's \href{http://gregmankiw.blogspot.com/}{blog}. \\

\noindent
United Nations Department of Economic and Social Affairs, Statistics Division: \\ 
\url{unstats.un.org} \\
United States Department of Commerce, Bureau of Economic Analysis: \\ \url{www.bea.gov} 
...
\item What is your ideal income? Why? How do you plan to pursue this?
      ...
      How do you plan to allocate your income between consumption and residential 
      fixed investment?
...
\item Extensions have been proposed for GDP that add environmental variables, 
      such as natural capital, to GDP. 
      (See, for example, the UN System of Environmental-Economic Accounting, 
      \url{seea.un.org}.)
      Consider extensions to GDP that would add \emph{social capital} to GDP. 
...
\item GDP measures income, that is, what we produce in a year.  
      But social standing doesn’t just depend on our income this year; wealth also matters.
      % GDP, because it represents consumption, is only a partial indication of wealth. 
      % For instance, owning an asset, like land or a valuable painting, doesn’t factor into GDP. 
      Thus, GDP may be less useful than measures of wealth for discussions of 
      social sustainability. 
      \begin{enumerate}
      \item How are wealth inequality and income inequality different? 
      Which do you think is worse from a social perspective and why? 
      \item Consider those who might be asset rich and income poor, like farmers,
      ...
      \item How might your notions of wealth and income inequality scale to the 
      macroeconomic level?
      \end{enumerate}

\item Is there such a thing as a steady-state economy? 
      What would have to be true for a steady-state economy to exist?
      Under what conditions is a steady-state economy sustainable?
...
\item A small example of (an attempted) circular economy is that
      some (high-end!) apparel manufacturer(s)
      will accept the worn-out garment back for recycling. 
      \begin{enumerate}
      ...
      such garments as opposed to more bargain clothing (make reference to Equation \ref{eq:IPARX}
      in your answer). 
      \item Is clothing a good candidate for a circular economy?
...
\item Social, economic, and environmental policies have effects that are 
      ...
      For example, air travel results in substantial greenhouse gas emissions.
      Therefore, banning air travel might substantially reduce greenhouse
      gas emissions. 
      ...
      A reduction in greenhouse gases is not given in dollars but could (arguably)
...
\item Sometimes you'll hear (well meaning) people blame corporations for wanting
      to ``maximize profit.'' One way of evaluating this point of view is to realize that 
      ...
      To what extent should we be as efficient as possible? In what ways? 
      Are some areas or domains ``off-limits'' to efficiency maximizing and what are they? Why?
      (Other approaches to this question are presented in Chapter \ref{chap:government}.)
      
\item IPCC6's \emph{most optimistic} scenario~\cite{IPCC:2018aa} is a 0.5~m 
...
\item What are ways that governments try to control the rate of economic growth (in~\%/yr)?
...
\item Research an individual or group who has chosen an off-grid or otherwise
      ...
      Suggested subjects include the Amish, the Lykov family, and Richard Proenneke.
      To what extent are they able to avoid interacting with the broader economic system?
      Pay particular attention to medical care, food, clothing, education,
      literature, and tools/machines, all of which are difficult to be completely 
      self-sufficient with respect to.
      Furthermore, what starting ``wealth'' (in whatever form) did these people 
      have, versus what ``income'' did they/do they generate to add to or subtract
