...
For instance, when travelling from point A to point B, 
...
Transportation (the purpose) is an example of an ``energy service.''
...
The numerator of the impact intensity of resources~\Xparen{} 
...
additional resources.
That leaves efficacy as the primary means of boosting affluence.
From a sustainability point of view, we should want everything to be used
as efficaciously as possible; 
doing otherwise is undesirable.
...
is needed to maintain capital stocks. 
...
There are substantial social and political pressures against degrowth---and for good reason!
...
Thus, the doughnut economy model attempts to correct 
both the flaws of a steady-state economy, namely that
...
A better approach is to address, for example, CO$_2$ emissions directly.
...
\item Economist Robert Gordon argues that GDP vastly underestimates 
      the benefits to humanity of innovation and new technologies~\cite{Gordon2016}.
      One example could be innovations in clothing over recent decades.
      Although the design of clothing (shirts, blouses, trousers, pants)
      remains mostly unchanged since the 1960s, 
      we now benefit from enhanced comfort due to improved fabrics.
      And yet, these new fabrics produce plastics pollution in the form of microfibers
      that enter waterways, lakes, oceans, and soil.
      So the new fabrics make our lives better with the negative externality
      of microfiber pollution.
      What can be done to address these environmental impacts?
      What resources (monetary and otherwise) 
      would be required to reduce microfiber pollution?
      How do you view the tradeoff of having inexpensive, comfortable clothes
      vs.\ having microplastic pollution?
      On the balance, has the innovation of synthetic fibers 
