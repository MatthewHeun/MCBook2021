...
<<AffluenceGraph-Nordhaus, fig.cap = "Annual world GDP per capita, affluence ($A$) in Equation~\\ref{eq:IPARC}. Sources: data for 1990--2020 from the \\citeauthor{World-Bank:2021un}~\\cite{World-Bank:2021un}; data for 1800--1989 inferred from GDP per capita growth rates from \\citeauthor{Nordhaus:1997vi}~\\cite{Nordhaus:1997vi} via \\citeauthor{DeLong:1997ui}~\\cite{DeLong:1997ui}.">>=
