...
but mostly wealth refers to the amount of goods, possessions, money,
property, or ``stuff'' owned by a person.
In other words, wealth refers to a level of material possessions or resources.
...
Both world GDP and affluence \Aparen{} have been growing exponentially 
...
Investment~(\mft{i}\;\!) is not money put into a bank account.
...
Net exports~(\mft{nx}) are the difference between the value of exports and the value of imports.
...
Capital is buildings, equipment, machines, and tools---things
...
More capital investment sometimes means greater economic output.
...
At some point, there is a maximum level of capital productivity.
Beyond that maximum,
...
also end up being about some form of productivity.
...
that is, sometimes reducing \mft{A} increases (at least some parts of) \mft{I}.
...
Conversely, although happiness can be found in the midst of financial hardship or poverty, 
...
Globally, social sustainability is often predicated on increasing affluence.
...
\item Extensions have been proposed for GDP that add environmental variables, 
      ...
      Consider extensions to GDP that would add social capital to GDP. 
...
\item One strategy to make electricity grids more efficient is to employ load shedding.
      ...
      air-conditioning unit. How much of a discount would it take for you
...
\item Sometimes you'll hear (well meaning) people blame corporations for wanting
      ...
      not maximizing profit when you have the opportunity to do so is 
...
\item IPCC6's most optimistic scenario~\cite{IPCC:2018aa} is a 0.5~m 
