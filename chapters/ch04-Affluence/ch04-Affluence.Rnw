...
property, or ``stuff'' a person has.
...
This economic activity may mean literal consumption, as in the case with food, 
or it could be using a service, such as education or getting a haircut. 
...
Affluence ($A$) is \emph{measured} by gross domestic product (GDP) per capita.
There are critiques as to why per capita GDP is not the \emph{best possible}
...
GDP is what we have the most data for and, consequently, it is what ``everyone'' uses. 
GDP is the \emph{de facto} standard. 
...
adjustment to (the measure of) resource intensity $R$. 
The end result leaves us in the same difficult spot: greater affluence 
creates greater impact if the resource intensity of the economy ($R$)
...
purposely devoted to increasing affluence and maintaining continual growth for 
GDP -- as shown in these Figures -- even though perpetual growth on finite 
resources is ultimately unsustainable (see Section \ref{sec:exp}).
...
\label{eq:GDP}
  GDP = c+g+i+nx \; .
\end{equation}
\paragraph{Consumption} $c$ is purchases made by individuals. 
...
Services represent nontangible things, such as a haircut, legal advice, education, 
and concert tickets. 
...
\paragraph{Government spending} $g$ is divided between government consumption 
(such as equipment and wages paid to government workers) and net investment, for 
example, infrastructure improvements.

\paragraph{Investment} $i$ is \emph{not} money put into a bank account. 
...
\paragraph{Net exports} $nx$ is the \emph{net} difference between the value of exports 
and imports. 
...
we use the term more colloquially as the sum of personal consumption, 
...
% supply and demand.
%
Therefore, we will also use the terms ``consumption'' and ``production'' interchangeably.
...
Total consumption, that is, world GDP, is $P \times A$.
Resources ($R$) are the feed the economy needs to live and grow.
The impact intensity of resources, $X$, is tied to the wastes that the economy excretes.
...
Either the economy uses \emph{more} inputs or it uses existing inputs more 
\emph{efficiently}.
...
Capital is buildings and equipment, machines, and tools -- \emph{things}
...
Efficiency improvements allow us to get more GDP for the same resource cost.
