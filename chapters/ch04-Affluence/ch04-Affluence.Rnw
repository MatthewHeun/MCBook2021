...
If affluence~\Aparen{} is a cause of impacts~\Iparen{},
then reducing \mft{A} is one way to reduce
\mft{I}, as Figure~\ref{fig:CarbonEmissionsGraph} shows for CO$_2$ emissions.
...
exceed the ecosystem's ability to absorb its wastes.
...
With a stable population, people in a steady-state economy would experience no change
...
because the size of the economy (in \$/year)is fixed.
...
For those economies, a transition to steady state can be a \emph{first step} 
