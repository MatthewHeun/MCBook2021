...
****
Managing the ``rebound effect,'' which is when improvements in efficiency result in
greater total consumption, is a major sustainability challenge.

***** Would it be possible to also talk about economic contraction here? ---MKH *****
...
(see Project~P1.\ref{project:alt_exp} for the relationship between continual growth
and exponential growth).
As has been mentioned a few times already, continual growth is inherently
...
or the ecosystem's regulating and support services pass a tipping point, as 
...
These proposals address economic growth, that is the changing size of the 
economy, but they don't address what that size is.
It is the magnitude of economic activity (that is, the total amount of consumption) 
that matters for sustainability -- as exemplified in Figure \ref{fig:FootprintHDIGraph},
which suggests that our current level of economic activity is \emph{already} too large.

Therefore, some proposals go further and call for \emph{de-growth}, that is, 
reducing the size of the economy.
...
Another alternative is a \emph{circular} economy\index{economy, circular}, 
which is one that exists only on recycled (and, perhaps, renewable) materials 
and which emits no wastes. 
...
Note the fact that we have \emph{desires} for more don't necessarily make us happier
when those desires are fulfilled.
Conversely, even though people \emph{can} find happiness in the midst of hardship,
want, or poverty, doesn't mean they desire those things.
Human want extends to choice as well. 
We would like to be able to choose what (and how much) we consume and to control
our world, but choice and control don't necessarily make us happier, per se.

Therefore, any reduction in affluence or choice runs counter to what people want.
...
system; see P4.\ref{project:off-grid} below for examples.
Often, for these people, independence or being ``off grid'' is the main attraction.
However, even for these people, there is no escaping the relationship to rest of
humanity.
There is always some participation in the broader economy, as the project shows,
...
United States Department of Commerce, Bureau of Economic Analysis: \\ \url{https://www.bea.gov/} 
