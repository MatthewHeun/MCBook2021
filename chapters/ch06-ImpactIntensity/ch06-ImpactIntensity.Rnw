...
is the CO$_2$ intensity of energy, $X_{CO_2}$.
The units for $X_{CO_2}$ are,
...
<<ImpactIntensityGraph, fig.cap="Carbon intensity of energy, $X_{CO_2}$. The line represents the ratio of Figure~\\ref{fig:CarbonEmissionsGraph} and Figure~\\ref{fig:PrimaryEnergyGraph} in each year.">>=
...
affluence ($A$),
energy intensity of the economy ($R_{Ep}$, the prime example for $R$)
\emph{or} carbon intensity of energy $X_{CO_2}$ (the prime example for $X$)
go to zero,
