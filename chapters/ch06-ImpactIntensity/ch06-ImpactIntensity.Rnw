...
\begin{equation}\label{eq:X}
  X = \frac{\textrm{total impacts produced}}{\textrm{total resources extracted}} \; .
\end{equation}
%
...
<<ImpactIntensityGraph, fig.cap="Carbon intensity of energy, $I_{CO_2}$. The line represents the ratio of Figure~\\ref{fig:CarbonEmissionsGraph} and Figure~\\ref{fig:PrimaryEnergyGraph} in each year.">>=
...
print(xtable::xtable(iparx_df %>% 
                     ...
                     caption = "IPARX terms from selected years in Figures~\\ref{fig:CarbonEmissionsGraph}, \\ref{fig:PopulationGraph}, \\ref{fig:AffluenceGraph-Nordhaus}, \\ref{fig:EnergyIntensityGraph-Nordhaus}, and~\\ref{fig:ImpactIntensityGraph}.",
