...
the EROI\index{energy return on investment (EROI)} for corn ethanol is typically less than 2~\cite{Hammerschlag:2006uf};
...
by liquid fossil fuels (gasoline and diesel) and illustrates two options for 
