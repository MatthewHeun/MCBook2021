...
\item Could driving be more environmentally friendly than walking? 
      ...
      The analysis was for a relatively short trip --- 2.5 km (1.5 miles) ---
      round trip, like walking to a store and back. The analysis included the 
      difference between the metabolic rate for driving and walking. 
      The analysis explicitly excluded the effects of embodied emissions in the 
...
\item As a preview of chapter \ref{chap:land_use_urban_planning}, there are strong
      ...
      Housing markets are regional \emph{because}, and to the extent that it is 
      ...
      in part because of reduced physical activity and reduced time for other 
      activities, which lead to stress. 
      One author reports eliminating two hours of commuting per day as equivalent
...
\item Congestion pricing attempts to attenuate demand for a public good by manipulating price.
      ...
      for all vehicle traffic (for example, including bus traffic).
      Some common objections relate to (hypothetical) displaced traffic to suburbs,
      ...
      Fundamentally, does it make sense --- if roads are a \emph{public} good ---
      to intentionally restrict their use? Why or why not? 
      Who gets to decide, for example residents or visitors? Who should get to decide?
      Should your (nearest) local city implement congestion pricing? Why or why not?
      What are the most important objective(s)?
      Which reason or reasons are most important?

\item Stroads and unsafe by design. Many ways to (implicitly) calculate the value 
      of a human life. Commute time again.
