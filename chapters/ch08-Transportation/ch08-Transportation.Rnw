...
% \begin{figure}
% \centering
% \includegraphics[width=\figwidth]{chapters/ch08-Transportation/figures/Total_US_tonne-kilometers_of_freight.pdf}
% \caption{Tonne-kilometers of freight by transport mode.}
...
% 
%   ggplot2::ggplot(mapping = ggplot2::aes(x = Year, y = value, linetype = type, colour=type)) +
%   ggplot2::geom_line() +
%   ggplot2::labs(x = ggplot2::element_blank(), y = expression("CO2 by transportation mode")) +
%   ggplot2::scale_x_continuous(breaks = c(1990, 2000, 2010, 2014)) +
%   ggplot2::labs(fill = element_blank()) +
%   MKHthemes::xy_theme()
% 
% @

% <<Transportation_Percentages_Mode, echo = FALSE, fig.cap = "Share of commuting distance by mode.", fig.width = 5, fig.height = 2.5>>=
...
% # Make the graph
% Transp_Pct %>%
%   ggplot2::ggplot(mapping = ggplot2::aes(x = "", y = percentages, fill = transportation_mode)) +
%   ggplot2::geom_bar(width = 1, stat = "identity") +
%   ggplot2::coord_polar("y", start = 0, direction = -1) +
%   ggplot2::geom_text(mapping = ggplot2::aes(y = label_ypos, label = paste(percentages, "%"), angle = ang),
%                      color = "white", size = 3) +
...
%   ) %>%
%   ggplot2::ggplot()+
%   ggplot2::geom_bar(ggplot2::aes(y=Percentage, x=Modes, fill = Modes),
%            stat="identity")+
%   ggplot2::geom_text(ggplot2::aes(y=Percentage, x=Modes, label=Percentage, group=Modes), color="black")
% @
% 
...
When 76.4~\% of all commuting kilometers are taken in single-occupant vehicles,
...
\item Energy consumption for last-mile deliveries has been increasing exponentially
      in recent years. 
      Research the rates of growth for e-commerce deliveries and the energy/CO$_2$
