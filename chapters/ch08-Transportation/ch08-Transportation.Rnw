...
Figure~\ref{fig:LLNL_sankey} shows that energy for transportation is 
supplied almost exclusively by fossil liquid petroleum fuels,
...
This chapter, and the two following chapters, will consider the many ways in which
...
Transport service levels are usually quantified in
...
% \begin{figure}
% \centering
% \includegraphics[width=\figwidth]{chapters/ch08-Transportation/figures/passengerkm_by_mode.pdf}
% \caption{Passenger-km of personal transport by mode.}
...
<<tonnekm_by_mode, fig.cap="Ton-miles of freight transport by mode, 1980--2018. Source: U.S.\\ Bureau of Transportation Statistics~\\cite{U.S.-Department-of-Transportation-Bureau-of-Transportation-Statistics:2021vu}.">>=
...
At the macro scale, 
economic growth is a large driver of demand for transport.
...
(b) a recession (2009) and a pandemic (2020) are good ways
to reduce demand for transportation services.
In fact, the correlation between economic growth and freight transportation growth
is so tight that the BTS claims that transport is a good indicator for economic activity
...
<<TSI, fig.cap="Seasonally-adjusted transportation index data for freight, passenger, and overall services. January 2005--December 2007 are highlighted with vertical lines. Source: Bureau of Transportation Statistics~\\cite{U.S.-Department-of-Transportation-Bureau-of-Transportation-Statistics:2021uk}.">>=
...
TSI %>% 
  ...
  ggplot2::scale_linetype_manual(values = c(Freight = 3, Passenger = 2, Overall = 1)) +
  ggplot2::scale_size_manual(values = c(Freight = 0.5, Passenger = 0.5, Overall = 1)) +
  ggplot2::labs(x = ggplot2::element_blank(), 
                y = "Transportation Services Index [2000 = 100]", 
...
from a society-wide transition to commuting modes different from driving alone.

\begin{figure}
\centering
\includegraphics[width=1\linewidth]{chapters/ch08-Transportation/figures/Figure 2-6 Commute Mode Share 2015.JPG}
\caption{Percentage of workers using various modes of transportation for commutes to work. Source: Bureau of Transportation Statistics \url{https://www.bts.gov/content/commute-mode-share-2015}.}
\label{fig:commuter_percentage_by_mode}
\end{figure}

% An important factor for transportation demand is fuel price. 
...
in recent years.
One of the most important aspects of freight transportation is the 
``last mile problem''~\cite{World-Economic-Forum:2020wf}, 
...
is the CO$_2$ emissions peaked in 2005, two years before the beginning of the Great Recession (2007--2009).
The CO$_2$ emissions peak was caused by a combination of factors:
...
which led to flattening of total CO$_2$ emissions, 
even as passenger transportation services rose through the 2005--2007 timeframe (Figure~\ref{fig:TSI}).


\begin{figure}
\centering
\includegraphics[width=\figwidth]{chapters/ch08-Transportation/figures/co2_emissions_by_transportation_mode.pdf}
\caption{CO$_2$ emissions by transportation mode.}
...
When 76.4~\% of all commutes are taken in single-occupant vehicles,
...
is the \emph{last mile problem}\index{last mile problem},
...
% <<Last_mile, echo = FALSE, fig.cap = "Last mile costs.">>=
% last_mile <- NULL
% percentages <- NULL
% label_ypos <- NULL
% ang <- NULL
...
% 
% Delivery_Cost %>%
%   ggplot2::ggplot(mapping = ggplot2::aes(x = "", y = percentages, fill =last_mile)) +
%   ggplot2::geom_bar(width = 1, stat = "identity") +
%   ggplot2::coord_polar("y", start = 0, direction = -1) +
%   ggplot2::geom_text(mapping = ggplot2::aes(y = label_ypos, label = paste(percentages, "%"), angle = ang), color = "white", size = 3) +
...
<<Biofuel_cons, fig.cap = "Biofuel consumption in the U.S., 1985--2017.">>=
...
2004	498\n2005	574\n2006	766\n2007	983\n2008	1357\n2009	1553\n2010	1821\n2011	1933\n2012	1892\n2013	2007\n2014	2067\n2015	2145\n2016	2279\n2017	2302\n"
utils::read.table(text = data_string, sep = "\t", header = TRUE, stringsAsFactors = FALSE, check.names = FALSE) %>%
  ggplot2::ggplot(mapping = ggplot2::aes(x = Year, y = `Biofuels [Trillion BTU]`)) +
  ggplot2::geom_line() +
  ggplot2::labs(x = ggplot2::element_blank(), y = expression("U.S. biofuel consumption [" * 10^12 * " BTU]")) +
  ggplot2::scale_x_continuous(breaks = c(1985, 1990, 2000, 2010, 2017)) +
  MKHthemes::xy_theme()
@
...
<<auto_gwp, fig.cap = "Environmental impacts of three different automobiles. Data from~\\citet[Fig.~1]{Hawkins_2012}. GWP is global warming potential; MRD is mineral resource depletion; ICE is internal combustion engine; EV is electric vehicle; Euro indicates the European electricity mix; use phase excludes fuels.">>=
...
utils::read.table(text = data_string, sep = "\t", header = TRUE, stringsAsFactors = FALSE) %>%
  ...
  ggplot2::scale_fill_discrete(name = element_blank()) +
