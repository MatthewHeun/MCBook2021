...
Today, transportation services are provided mostly 
...
demand for transportation services and discusses sources of that demand.
...
output called the ``Transportation Services Index'' 
(TSI)~\cite{U.S.-Department-of-Transportation-Bureau-of-Transportation-Statistics:2021uk},\index{transportation services index}
...
indicates that large reductions to energy consumption and CO$_2$ emissions could be obtained 
...
the next section discusses sustainability challenges associated with 
transport.
...
A sustainability challenge for the last mile problem is 
``how can we minimize CO$_2$ emissions from last mile deliveries
...
A sustainability challenge for the first mile problem is
``how can we solve the first mile problem and maintain livable spaces?''
...
Later, when coal emerged as a fuel for home heating, power generation, and
industrial processes, coal-derived PM and sulfur compounds were additional concerns. 
The introduction of ``dirty'' internal combustion engines 
for transport intensified air-quality issues, particularly in urban areas.
...
Global ammonia emissions, a key contributor to PM, 
continue to grow, however.
The sustainability challenge with PM is 
``can we eliminate PM emissions while providing heated spaces and transportation 
...
The fourth sustainability challenge of transportation is CO$_2$ emissions.
...
even as consumption of transportation services rose through the 2000s;
see Figure~\ref{fig:TSI}.
A sustainability challenge for CO$_2$ emissions is
``how can we quickly disseminate CO$_2$-free transportation options
across all income levels of society?''
...
Vehicle fuel economy (an efficacy, not an efficiency)
...
the impact intensity of resource extraction and consumption
...
Furthermore, ethanol production is very land-intensive.
...
In the case of the Ren20 electricity mix,
