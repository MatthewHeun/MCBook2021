...
Global $SO_2$ emissions peaked about 1990 and global $NO_x$ emissions peaked around
2018. 
...
******** Update and get source. Put in Excel format. ********)

<<US_mpg, fig.cap = "U.S.\\ car and truck fuel efficiency">>=
data_string <-
"Year	Type	MPG\n
1975	Cars	13.5\n
1976	Cars	14.9\n
...
utils::read.table(text = data_string, sep = "\t", header = TRUE, stringsAsFactors = FALSE, check.names = FALSE) %>%

  ggplot2::ggplot(mapping = ggplot2::aes(x = Year, y = MPG, linetype = Type)) +
  ggplot2::geom_vline(xintercept = 2005, colour = "gray", size = 0.2) +
  ggplot2::geom_vline(xintercept = 2008, colour = "gray", size = 0.2) +
  ggplot2::geom_line() +
  ggplot2::labs(x = ggplot2::element_blank(), y = expression("Miles per gallon")) +
  ggplot2::scale_x_continuous(breaks = c(1975, 1980, 1990, 2000, 2010, 2016)) +
...
\item Could driving be more environmentally friendly than walking? 
      ...
      What boundaries or assumptions are you making?
...
\item Strong Towns defines a road as a high-speed connection between two places,
      ...
      Pedestrian fatalities increased from 4,302 in 2010 to 6,374 in 2019,
...
% https://arstechnica.com/cars/2021/03/us-pedestrian-deaths-rose-sharply-in-the-first-half-of-2020
      Preliminary data for 2020 from the \href{https://www.ghsa.org/resources/Pedestrians21}
      {Govenor's Highway Safety Association} reports a further 4.8\% increase in pedestrian 
      deaths over 2019.
      \href{https://smartgrowthamerica.org/dangerous-by-design/}{Dangerous by Design 2021}
      ...
      Second, traffic flow design faces a fundamental tradeoff between traffic volume (speed)
      ...
      However, we have \emph{de facto} estimates on the worth of a human life;
      for simplicity, let's use the \$10 million estimate~\cite{NPR_4_23_2020}.
      ...
      additional commute? 
      \item Is this how we should do street, road, and stroad design?
