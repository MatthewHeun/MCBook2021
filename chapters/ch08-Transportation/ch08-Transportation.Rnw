...
physical location, economic activity, energy, and resources are interrelated.
\end{mcframe}

This chapter begins by discussing
demand for transportation services and
drivers for that demand.
...
\emph{if}| the electricity source includes renewables (as in the ``Euro'' mix), 
...
\item E-commerce deliveries have been increasing prominent recently. 
      How does this compare to consumers traveling to stores to make purchases?
      What if e-commerce delivery meant that purchases were delivered to a 
      neighborhood location from which customers had to pick packages?
      Consider the implications of these alternatives.
