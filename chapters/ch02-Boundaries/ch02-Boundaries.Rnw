...
<<FootprintHDIGraph, fig.cap="Ecological footprint vs. HDI.">>=
readxl::read_excel(file.path("chapters", "ch02-Boundaries", "datasets", "EFvsHDI.xlsx"),
                                  sheet = "data to plot") %>%
  dplyr::filter(`Country Name` != "World") %>% 
  dplyr::mutate(
    Population = Population / 1e6 # Population now in mullions
  ) %>%
  ...
  ggplot2::scale_shape_manual(values = c(Africa = 16, Asia = 1, Europe = 16, 
                                         `North America` = 16, Oceania = 16, `South America` = 16)) +
  ggplot2::scale_alpha_manual(values = c(Africa = 0.5, Asia = 1, Europe = 0.5,
                                         `North America` = 0.5, Oceania = 0.5, `South America` = 0.5)) +
