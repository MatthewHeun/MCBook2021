...
While the risk is, perhaps, less acute than it once was, a 
...
The second method uses a weighted average of impacts, called an {\i index}, 
and the idea is to minimize the level of the index.
...
Climate scientists have proposed %\cite here Rockstrom 2009 = one of the first 4 in the wikipedia article.
...
The boundaries are intended to define a "safe operating space for humanity."
...
They are (weighted) averages of quantities related to
...
readxl::read_excel(file.path("chapters", "ch02-Boundaries", "datasets", "EFvsHDI.xlsx"),
  ...
  ggplot2::scale_size_continuous(breaks = c(1e5, 1e6, 1e7, 1e8, 1e9, 1e10)) +
