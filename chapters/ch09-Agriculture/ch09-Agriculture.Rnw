...
neighbors.
However, the most globally important form of air emissions is of greenhouse gases\index{greenhouse gases}.

A second sustainability challenge that arises in food and agriculture
is the greenhouse gas
emissions whose origin is the food delivery system.
Figure~\ref{fig:FoodEmissionsPerKg} shows the amount of CO$_2$ released 
from the production and distribution of one kilogram 
of several types of food~\cite{Ritchie:2021tl}.


see Figures~\ref{fig:FoodEmissionsPerKg} and~\ref{fig:GHG_food}.
