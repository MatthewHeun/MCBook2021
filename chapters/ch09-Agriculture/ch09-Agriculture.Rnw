...
ag_sankey_data <- readxl::read_excel(file.path("chapters", "ch09-Agriculture", "datasets", "Ag Sankey.xlsx"),
                                     sheet = "data to plot") %>% 
  dplyr::mutate(
    node = factor(node, levels = c("Cultivated fields", 
                                   "Grains", "Soybeans", "Fruits", "Sugarbeets", "Vegetables", "Other oilseeds",
                                   "Available crops",
                                   "Stored grains & soybeans",
                                   "Storage"))
  )
...
<<food_and_ag_material_flow_d3, fig.cap="Caption">>=
library(networkD3)
...
networkD3::sankeyNetwork(Links = ag_sankey_links, Nodes = ag_sankey_nodes, 
                         ...
                         Value = "value", NodeID = "node_id",
                         NodeGroup = "node_group", LinkGroup = "link_group")
