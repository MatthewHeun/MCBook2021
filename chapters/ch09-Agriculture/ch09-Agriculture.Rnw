...
The paragraphs below will look at two very important greenhouse gases, carbon
...
paint a suggestive (but incomplete! ***why incomplete?***) picture of agricultural
CO$_2$ emissions.
...
and impacts produced) or, equivalently, it must increase its productivity.
To be strongly sustainable, agriculture must increase soil fertility without extracting minerals and 
water from the environment at rates greater than their regeneration rates.
The sections below examine agricultural efficacy and soil regeneration in more detail.

%++++++++++++++++++++++++++++++
\subsection{Agriculture efficacy metrics }
\label{sec:ag_metrics}
%++++++++++++++++++++++++++++++

Figure~\ref{fig:food_and_ag_material_flow} shows a mass-based efficacy for U.S.\
agriculture, which is one aspect of the resource intensity~\Rparen{} of agriculture.
...
calories.
In addition, neither of these measures provides guidance for improving productivity
...
which boosts the productivity of the panels.
The panels provide shade for crops, which lessens the amount of water needed for irrigation.
...
Any measure that will increase the efficacy of the food system is a steps towards
...
Agriculture is an energy sink rather than an energy source and requires the 
unsustainable input of fossil fuels.
