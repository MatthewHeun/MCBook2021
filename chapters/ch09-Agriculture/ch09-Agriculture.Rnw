...
food calories, for example) scales linearly with population. 
...
impacts it produces are explored more in the following sections.
...
nitrogen (N), potassium (K), and phosphorous (P). Agrichemicals include herbicides,
pesticides, and fungicides. To the extent that any of these are derived from 
nonrenewable feedstocks, such as fossil fuels, they are not strongly sustainable.
...
return on investment of food (see Section~\ref{sec:masterE}).
EROI greater than one indicates that food is an energy source for humans, that is,
we obtain more energy from the food that it took to get the food.
EROI less than on indicates we \emph{lose} energy getting the food. 
However, since we have to have food, food is worth spending energy on.
Whether getting food in this way is sustainable or not depend on where our energy
comes from. 
...
Here we list some (not all) of the outputs, or by products, from agriculture and 
...
neighbors.
However, the most globally important form of air emissions is of greenhouse gases,
see Figures~\ref{fig:pounds_serving}, \ref{fig:GHG_food}, and~\ref{fig:GHG_from_food}.

<<pounds_serving, fig.cap="Carbon dioxide equivalent emissions per serving of various foods~\\cite{Heller_2014}.">>=
data_string <-
...
read.table(text = data_string, sep = "\t", header = TRUE, stringsAsFactors = FALSE, check.names = FALSE) %>%
  ...
  ggplot2::ggplot() +
  ggplot2::geom_bar(ggplot2::aes(y = `kg CO2 per serving`, x = Product), stat = "identity", fill = mc_title_blue) +
  labs(x = ggplot2::element_blank(),
       y = expression("Emissions [kg " * CO[2] * "/serving]"),
...
<<FoodEmissionsPerKg, fig.height=4, fig.cap="Food CO$_2$ emissions per kg. Source: Our World in Data~\\cite{Ritchie:2021tl}.">>=
...
A second sustainabilty challenge that arises in food and agriculture
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Efficiency metrics (cal/acre, EROI, \$/acre). Land allocation to different crop types (see land use figure)
Agriculture uses 70\% of water. About 2\% of US emissions are food that is discarded.
\begin{figure}
\centering
...
a succinct yet complete introduction to some of the issues covered
in this chapter.
...
\item Food exports (or imports) are energy (or water) exports (or imports).
      How does this look from an energy (or water) efficiency standpoint?
      Money is the mechanism that we use to value economic transactions. What
      might be an analogous denomination for environmental or social value?
