...
<<food_and_ag_energy_flow, fig.scap="Energy consumed in the food-delivery system by life-cycle phase", fig.cap="Amount of energy consumed in the U.W.\\ food-delivery system by life-cycle phase (2021). \\emph{Source}: \\citet{US_food}.">>=
...
The EROI of the food-delivery system is \Sexpr{format(food_eroi * 100, digits = 2)}\%.
With modern food system, it takes % \Sexpr{format(food_eroi_denominator, digits = 3)}
\Sexpr{sprintf(food_eroi_denominator, fmt = "%#.2f")}
calories of energy to provide \Sexpr{format(food_eroi_gross_output, digits = 3)}
calories of food energy, of which only \Sexpr{format(food_eroi_numerator, digits = 3)}
calories are consumed.
The remaining \Sexpr{format(food_eroi_waste, digits = 3)} are wasted.
