...
numerator <- food_energy %>% 
  dplyr::filter(Category == "Consumed") %>% 
  dplyr::select(value)
denominator <- food_energy %>% 
  dplyr::filter(type == "Input energy") %>% 
  dplyr::select(value) %>% 
  sum()
food_eroi <- numerator / denominator
...
With modern agriculture, it takes roughly 13 calories of energy to provide 1 calorie of consumed food energy.
...
and impacts produced) or, inversely,
it must increase its productivity.
