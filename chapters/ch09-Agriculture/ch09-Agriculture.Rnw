...
Instead, this chapter covers agriculture as an \emph{example} industrial/commercial
area. 
Agriculture is a setting most readers will have (at least) an intuitive understanding of.
Agriculture, forestry, and other land use together account for an estimated 23\% 
of anthropogenic greenhouse gas emissions~\cite{SRCCL2019}. 
Furthermore, agriculture has strong links to land use, which is the focus of 
...
with an estimated 12\% of global ice-free land being used for crops, 37\% for 
pasture, and 22\% as forests for timber production~\cite{SRCCL2019}.
...
Furthermore, Section~\ref{sec:malthus_erlich_simon} indirectly suggests humanity 
...
Equation~\ref{eq:IPARC} indicates reductions in agricultural impacts on the 
...
of agriculture, the resource intensity (R) or the contamination intensity (C).
...
When considering whether an activity is sustainable, input-output analysis is an 
essential tool.
...
However, accurate and reliable results depend on an accurate accounting of inputs and outputs.
...
consideration and is, perhaps, more difficult than for other areas of sustainability 
(at least judging by the many conflicting analyses in the literature).
Recall the definitions for weak and strong sustainability from 
...
\subsection{Agricultural inputs}
Here we list some (not all) of the inputs required for agriculture and consider their
sustainability implications.
\paragraph{Soil} As mentioned earlier, humans devote a considerable portion of 
...
\paragraph{Light} Plants need light to grow.
...
\paragraph{Water} Rain water is an ecosystem service. 
...
\paragraph{Fertilizers and agrichemicals} The most common fertilizers are contain
...
\paragraph{Energy} The energy input to agriculture ranges from human and animal 
...
return on investment (ERIO)\index{EROI} of food (see Section~\ref{sec:masterE}).
...
An EROI less than on indicates we spend more energy getting the food than we get 
\emph{from} the food. 
...
\begin{figure}
\centering
\includegraphics[width=0.75\linewidth]{chapters/ch09-Agriculture/figures/Energy_flow_food_systems.pdf}
\caption[Energy Flow in the US.]{Amount of energy consumed in different aspects of 
food delivery in the U.S.~\citep[Fig.~2]{Heller_2000}. 
...
\subsection{Agricultural outputs}
Here we list some (not all) of the outputs, or byproducts, from agriculture and 
...
Figure~\ref{fig:energy_flow_in_food_system} energy use in various stages of food
...
Taken together, Figures~\ref{fig:energy_flow_in_food_system}, \ref{fig:FoodEmissionsPerKg},
and~\ref{fig:GHG_food} paint a suggestive (but incomplete!) narrative of agricultural
CO$_2$ emissions.

<<FoodEmissionsPerKg, fig.height=4, fig.cap="Food CO$_2$ emissions per kg of food. Source: Our World in Data~\\cite{Ritchie:2021tl}.">>=
...
food_emissions_per_kg %>% 
  ...
  ggplot2::geom_bar(stat = "identity", fill = mc_title_blue) +
  labs(x = ggplot2::element_blank(),
...
\begin{figure}
\centering
\includegraphics[width=0.75\linewidth]{chapters/ch09-Agriculture/figures/GHGfromFoodConsumption.pdf}
\caption[GHG emissions from food.]{Food greenhouse gas emissions~\citep[Fig.~2]{Heller_2014}.}
\label{fig:GHG_food}
\end{figure}

% <<GHG_from_food, echo = 'none', fig.cap = "Percentages of greenhouse gas emissions from food consumption.", fig.scap = "Food greenhouse gas percentages.", fig.width = 5, fig.height = 2.5>>=
...
\paragraph{Water} Agriculture can have significant impacts on water quality and quantity.
...
\paragraph{Land} The main impact of agriculture on soil is erosion.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Agricultural efficiency}
\label{sec:food_ag_efficiency}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{figure}
\centering
\includegraphics[width=0.75\linewidth]{chapters/ch09-Agriculture/figures/Material_Flow_in_the_US_Food_System(1995_Flows_in_Million_Pounds).pdf}
...
agriculture, which may, or may not, be relevant to sustainability.
...
Various energy-based metric could be applied to agriculture.
We could attempt to maximize the amount of food produced per energy consumed.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{A portrait of sustainable agriculture}
...
makes food more expensive, which is consistent with Equation~\ref{eq:IPARC}
(reduced impact (I) comes from reduced affluence (A)).
...
the American Supermarket}, Avery, 2020. \\
\noindent Gillian Crowther, \emph{Eating Culture: An Anthropological Guide to Food, 
2nd Edition}, University of Toronto Press, 2018.
...
\item One common metric for agricultural productivity is yield, the amount of a 
      ...
      Such a metric does not account for efficiency, intensity, or EROI.
      Of course, the most basic sustainability metric for agriculture is economic
      profitability, (net) profit per acre.
...
\item Consider the tension between feeding hungry people and environmental 
      impact. Supposed that you could save 100 lives by emitting an additional
      100 tons of CO$_2$ equivalent per year. 
...
\item Where should the environmental impact(s) from almond growing be placed, 
...
\item Would requiring an ``environmental scorecard'' label on each individual
      food item change consumer behavior? Why or why not? What would the cost be?
...
\item A vegan diet is only possible with supplementation of key critical nutrients,
      ...
      What roll should such \emph{industrial} food play in our food system, as opposed
      to ``natural'' food, such as what is farmed or harvested wild? 
...
\item A significant theme of Lorr's book \emph{The secret life of groceries} 
...
\item Verify the efficiency values in Section~\ref{sec:food_energy_return_ratios}. % {sec:food_ag_efficiency}?
