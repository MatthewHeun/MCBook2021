...
show that residences use 15.7\% of U.S.\ energy and are responsible for 19.3\% 
...
things that aren't food, such as wood, cellulose, sunlight, or petroleum, for example.
...
The economic value the agriculture, forestry, and fishing industry is 
...
affluence rises.
Thus, the global demand for food has two main drivers: population~\Pparen{} is the primary
driver of demand and affluence~\Aparen{} is the secondary driver.
...
Section~\ref{sec:electric_vehicles} considered the vehicles' emissions
while in use, the energy and mineral resources used to create each vehicle, and
the source of the power for each vehicle (Figure~\ref{fig:auto_gwp}). 
...
Sustainability requires not using renewable resources faster than they can regenerate
...
of the  ecosystem.
If agriculture decreases soil fertility, it is not sustainable.
If agriculture depends on a nonrenewable resource, such as phosphate fertilizers, it is not strongly sustainable.
...
As mentioned earlier, agriculture is the majority of human use of land.
Likewise, agriculture accounts for approximately 70\% of human fresh water use.
...
In particular, top soil contains organic mattered needed by plants.
However, top soil can be lost by erosion to the action of wind and/or water.
...
Loss of top soil reduces the productivity (in output per acre) of farmland.
...
Agricultural productivity can be enhanced with fertilizers and/or agrichemicals.
...
Phosphate fertilizers are necessary for high crop yields but they originate from 
