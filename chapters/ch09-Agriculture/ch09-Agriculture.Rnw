...
<<food_ghg, fig.cap="U.S.\\ food-related greenhouse has emissions by life cycle stage. \\emph{Source}: \\citet{Crippa:2021tp}.">>=
food_emissions_US <- readxl::read_excel(file.path("chapters", "ch09-Agriculture", "datasets", "Crippa et al 2021.xlsx"), 
  ...
  dplyr::mutate(
    ...
    `System Stage` = factor(`System Stage`, levels = c("Production", "Transport", "Retail", 
                                                       "LULUC (Production)", "Processing", 
                                                       "Packaging", "End_of_Life", 
                                                       "Consumption"))
  )
...
food_emissions_US %>% 
  ...
  ggplot2::labs(x = ggplot2::element_blank(), 
                y = "U.S. food-related GHG emissions [Gtonnes/year]") +
...
\caption[Material flow in the U.S.\ food system (1995, flows in million pounds.)]{Material flow in the U.S.\ food system (1995, flows in million pounds). \emph{Source}: \citet[Fig.~1]{Heller_2000}.}
