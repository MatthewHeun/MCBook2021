...
Demographic transition theory, illustrates schematically in Figure  \ref{fig:dtt},
is that economic development leads to a decrease in mortality followed some time 
...
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
This gives us some hope for sustainability, namely that population won't
...
\item The fear that population will outstrip resources is called \emph{Malthusian
      ...
      **** per James: consider normalacy bias. ****
