...
As shown in Equation \ref{eq:IPARC}, human population 
has a major impact on 

% Population is only "major" if its rate is large. 
% So suggest downplaying this a bit:
...
The point of Figure~\ref{fig:World3loops} isn't whether one model is right or wrong,
but rather that many variables that effect population.
