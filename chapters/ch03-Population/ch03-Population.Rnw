...
region_pop %>% 
  dplyr::filter(Country != "World", Year >= 1700) %>% 
  dplyr::mutate(
    Country = factor(Country, levels = c("Australia", "North America", "Latin America", "Europe", "Africa", "Asia"))
  ) %>% 
  ggplot2::ggplot() + 
  ggplot2::geom_area(mapping = ggplot2::aes_string(x = "Year", y = "total_population_billions", fill = "Country")) + 
  ggplot2::scale_x_continuous(limits = c(1700, NA)) +
