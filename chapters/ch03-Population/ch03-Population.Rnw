...
Wild vertebrate populations have declined by 68\% since 1970 \cite{WWFLPI2020}. 
...
We should distinguish between the {\i reason(s)} and the {\i mechanism(s)} for 
the decline in animal populations. Population decline is an {\i impact} from the left-hand
side of \ref{eq:IPARC}. The {\i reason(s)} (that is, the "why") are given on the right-hand side of \ref{eq:IPARC}.
In contrast, some of the {\i mechanisms} (that is, the "how") behind the decline 
...
whales, bison, aurochs, and most North American megafauna.
...
Figure \ref{fig:PopulationGraph} shows the Earth's human population from X to present.
