...
Equation \ref{eq:IPARX} expresses human impact on the Earth as the product
...
This chapter examines human population dynamics and the impact of human 
...
Through the twentieth century, the global human population grew exponentially,
which was unsustainable.
Currently, population growth has slowed (but not ceased).
Prospects for future human populations are discussed with an eye toward whether
humanity will live sustainably in the future. 
...
of less than 0.001\% of species going extinct per year. 
...
Population decline is an \emph{impact} ($I$) from the left-hand side of 
Equation \ref{eq:IPARX}. 
The \emph{reason(s)} (that is, the ``why'') are given on the right-hand side of
Equation \ref{eq:IPARX} ($PARC$).
...
Overconsumption by humans is another cause of population decline. 
Ocean fish stocks are the prime example.
...
Bees are a particularly alarming example because of the valuable role they play 
in pollination.
...
In July 2021, the world's human population is 7.7--7.9 billion with a
...
# region_pop_to_fit <- region_pop %>%
#   dplyr::filter(Year >= 1800, Country == "World")
# 
# world_pop_log_fit <- lm(data = region_pop_to_fit,
#                         formula = log(total_population) ~ Year)
# 
# world_pop_exp_fit <- nls(data = region_pop_to_fit,
#                          formula = total_population ~ p1*exp(r * (Year-1800)), start = list(p1 = 1e9,  r = 0.01))
# predicted_df <- tibble::tibble(Year = region_pop_to_fit$Year,
#                                pop_predict = predict(world_pop_exp_fit, newdata = world_pop_exp_fit$Year)) %>%
#   dplyr::mutate(
#     total_population_billions_fitted = pop_predict / 1e9,
#     Country = "World"
#   )
...
region_pop %>%
  ...
  ggplot2::geom_area() +
  # ggplot2::geom_line(data = predicted_df, mapping = ggplot2::aes_string(x = "Year", y = "total_population_billions_fitted")) +
...
<<ddt, fig.cap="Schematic of demographic transition theory. Crude birth rate (CBR) and crude death rate (CDR) are defined as births or deaths per year per 1000 people in the total population (uncorrected for sex or age cohorts).">>=
...
demo_transition %>% 
  ...
  ggplot2::labs(x = ggplot2::element_blank(), 
                y = "Birth and death rates [per 1000]", 
...
demo_transition %>% 
  ...
  ggplot2::ggplot(mapping = ggplot2::aes(x = Year, y = vals, linetype = which_line)) + 
  ggplot2::geom_line(colour = mc_title_blue, size = 1) + 
  ggplot2::labs(y = "Population [millions]", 
                linetype = ggplot2::element_blank()) +
  MKHthemes::xy_theme()
...
ggplot2::ggplot(data = world) +
  ...
  ggplot2::geom_point(data = mong_khet, ggplot2::aes_string(x = "lon", y = "lat"),
  ...
  ggplot2::geom_path(data = v_circle, mapping = aes_string(x = "lon", y = "lat"), colour = mc_title_blue, size = 1) +   
...
\noindent World3 model online: \\ \url{http://bit-player.org/wp-content/extras/limits/}
...
\item It is interesting to note that Malthus's book was published (1798) just before before 
      Figure~\ref{fig:PopulationGraph} begins (that is, 1800AD). 
