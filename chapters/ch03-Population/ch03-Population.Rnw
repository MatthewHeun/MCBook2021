...
%++++++++++++++++++++++++++++++
Whether the human population will continue to grow rapidly seems like a key 
...
<<UN_population_prospect, fig.cap="Projections of world population. Projections are based on probabilistic models of total fertility and life expectancy at birth from a Bayesian hierarchical model. The light gray region shows a 95~\\% prediction interval. The dark gray region shows a 80~\\% prediction interval. \\emph{Sources}: Historical data from \\citet{Roser:2021uu}; future projections from \\citet{UNEcSoAf:2021vj}.">>=
...
The point of Figure \ref{fig:World3loops} is to show that many variables affect 
...
the total human population ($P$) won't \textit{continue} to grow exponentially. 
On the other hand, slowing of growth is predicated on increasing GDP per capita
(the $A$ term in Equation~\ref{eq:IPARX}).
\end{mcframe}
...
Nevertheless, it is unwise to consider urban areas divorced from the rural
