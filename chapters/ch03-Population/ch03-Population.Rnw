...
alarming example because of the extremely valuable role they play in pollinating 
...
used in the World3 population model. 
Notice the number of variables (24!) and the many ways those variables (are 
thought to) interact.
...
In phase 3, changing economic conditions, such as increase urbanization, a 
transition away from an economy dependent on agriculture, and availability of 
contraception lead to a declining birth rate. 
In phase 3, the birth rate is still greater than the death rates so the population 
...
In phase 5, the birth rate can begin to increase again (as shown in Figure \ref{fig:dtt}) \emph{or}
...
All the next generation of parents have already been born. 
A falling birth rate doesn't immediately lower the population; it takes time for 
the population to stabilize. 
Likewise, a small cohort of parents, even if they have a high birth rate, cannot 
...
\item What are ways population could be controlled?
      **** Reference World3 model figure? Give some examples? China, USAID contraception? ****
      % something about replacement birth rate?
      
      % question about far-flung families and social connectedness. 
      % how do we find spouses and create families. 
