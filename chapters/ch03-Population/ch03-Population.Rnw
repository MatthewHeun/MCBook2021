...
and the impact \mft{I} of human activities on animal populations.
(``Human activities'' are a direct result of the size of \mft{P}.)
The decline of many animal populations is concerning (in part) because it likely 
indicates a reduction in ecosystem services.
Throughout history, the global human population \mft{P} has grown continually,
...
A key interaction is between the number of people and economic output per person.
...
Ocean fish stocks are a notable example, part because of the role they play in human diets.
...
The wide range of uncertainty in the population projections is caused by
uncertainty about future birth and death rates.
...
used in World3, an early (and simple) world population model. 
Figure~\ref{fig:World3loops} shows the large number (24) of variables in the 
model that affect population and the many ways those variables (are programmed to) interact.
...
population and those variables interact in complex ways.
...
The UN projections in Figure~\ref{fig:UN_population_prospect} are not based on 
the World3 model. 
Instead, the UN used a Bayesian model based on likely interactions among the 
...
In an undeveloped society, birth rates and death rates are high but balanced, 
...
%% MKH: 104 - 100 = 4. 4-3=1. Higher growth rate with bigger numbers.
Demographic transition theory, illustrated schematically in Figure~\ref{fig:dtt},
is the observed pattern that economic development leads to decreased mortality 
followed some time later by a falling birth rate.
Thus, population is predicted to grow rapidly and then stabilize.
...
the total human population \Iparen{} won't \textit{continue} to grow exponentially. 
