...
<<CO2-Savings, fig.scap="CO$_2$ savings from individual actions", fig.cap="CO$_2$ emission savings from example actions for rows in Table~\\ref{tab:personal_energy_actions}. ``No commute'' means living close enough to walk or bike to work instead of commuting 50~miles each-way, 230 times/year with a Honda Accord. ``Smaller home'' means living in a 1250~ft$^2$ home instead of a 2750~ft$^2$ home. ``Chicken, not steak'' means choosing 6~ounces of chicken instead of 6~ounces of steak 25 times per year. ``Accord, not F150'' means driving 12,000~miles/year with a Honda Accord instead of a Ford F150. ``No NYC$\\rightarrow$LHR'' means eliminating one New-York-to-London round trip flight in economy class per year. \\emph{Source}: own calculations.">>=
co2_savings <- readxl::read_excel(file.path("chapters", "ch12-PersonalAction", "datasets", "CO2Savings.xlsx"),
                                  sheet = "data to use") %>% 
  dplyr::mutate(
    Action = dplyr::case_when(
      ...
      Action == "Smaller home" ~ "Smaller~home", 
      Action == "Chicken, not steak" ~ "list(Chicken, not~steak)", 
      Action == "Small car" ~ "Small~car", 
      Action == "No NYC->LHR" ~ "No~NYC%->%LHR"
    ),
    Choice = dplyr::case_when(
      Choice == "100 miles/day" ~ "100~miles/day", 
      Choice == "Bike or walk" ~ "Bike~or~walk", 
    ...
    Action = factor(Action, levels = c("No~commute", "Smaller~home", "list(Chicken, not~steak)", 
                                       "Small~car", "No~NYC%->%LHR")), 
    Choice = factor(Choice, levels = c("100~miles/day", "Bike~or~walk", 
                                       "2750~ft^2", "1250~ft^2", 
                                       "Steak", "Chicken", 
                                       "F150", "Accord", 
                                       "NYC%->%LHR%->%NYC", "Zoom")),
