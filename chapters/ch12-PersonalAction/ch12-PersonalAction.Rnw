...
Edgar McGregor picked up litter in a popular Los Angeles county park at least one
...
\captionsetup[subfigure]{justification=centering}
     ...
     \end{subfigure}
        \caption{Two efficient commuting options.}
...
Figure~\ref{fig:gutter_diversion} shows one way of reducing the amount of surface
runoff that flows directly into local waterways.
