...
a single-minded difference-make like Edgar McGregor, 
a Nobel Peace Prize winner like Wangari Maathai, or 
a technical genius like Joseph Lstiburek.
Decisions we all make today will affect our environmental impacts for decades to come.
...
\item \label{question:mcgregror}
      While laudable and environmentally friendly, were Edgar McGregor's actions
...
\item \label{problem:more_actions} 
      Figure~\ref{fig:CO2-Savings} shows an example sustainability-beneficial action
      ...
      Then, think of one additional sustainability-related action 
      for each row.
