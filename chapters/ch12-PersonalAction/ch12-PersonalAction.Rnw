...
\captionsetup[subfigure]{justification=centering}
     ...
     \end{subfigure}
        \caption{Two (potentially) low-carbon commuting options. The Tesla shown
        ...
        (February 8, 2021).
        \emph{Sources}: 
        (a)~\copyright{}2019 Jeremy Van Antwerp;
...
\begin{table}
\caption[Personal Q\&A]{Individual actions Q\&A}
\centering
\begin{tabular}{l l}
\toprule
...
\begin{tabular}[c]{@{}l@{}}How close will I live to work, \\ school, church, and stores?\end{tabular} & Close enough to allow walking and cycling \\
...
Note: for
Q\ref{chap:personal_action}.\ref{question:mcgregror}--Q\ref{chap:personal_action}.\ref{question:lstiburek} 
it would be helpful to 
read more in depth about Edgar McGregor, \emph{Juliana v. United States},
Wangari Maathai, and Joseph Lstiburek.

\begin{questions}

\item While laudable and environmentally friendly, were Edgar McGregor's actions
      (see Section~\ref{sec:stories_individual_actions}) \emph{necessary} for 
      \emph{sustanability}?
...
\item Wangari Maathai (see Section~\ref{sec:stories_individual_actions}) was jailed
