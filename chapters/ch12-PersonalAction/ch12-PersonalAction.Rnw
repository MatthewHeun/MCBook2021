...
Individuals can choose to install renewable energy generation systems
...
(to avoid the rebound effect and backfire).
...
broader economy and via the rebound effect.
...
will prevent additional carbon being embodied in a new vehicle.
...
All food embodies resources (such as energy) and emissions.
Wasting food means that no benefits come from those embodied resources and emissions.
After minimizing food waste, 
individuals can adopt a climate-friendly or climate-friendlier diet.
Typically climate-friendly diets involve reductions to meat consumption, particularly beef.
...
Buy \emph{ingredients} (Figure~\ref{fig:suburban_produce_aisle}), then 
\emph{make} food.
Even better, buy food directly from local farmers, perhaps at a farmers' 
market (Figure~\ref{fig:urban_farmers_market})\index{farmers' market} or through community-supported 
agriculture\index{community supported agriculture}.
A further step is raising your own food by gardening or by harvesting 
wild food (by hunting, fishing, and/or gathering wild plants). 
...
record what inputs (water, fertilizer, your time) are needed, 
...
Advocate for local \emph{and} regional sustainability actions. 
...
Decisions made today will affect your environmental impacts for decades to come.
It may be helpful to ask the questions shown in Table~\ref{tab:personal_energy_actions}.
...
There are many additional actions for the rows in Table~\ref{tab:personal_energy_actions}.
P12.\ref{question:question:more_actions} provides an opportunity 
to explore additional individual sustainability actions.
...
\item \label{question:more_actions} Figure~\ref{fig:CO2-Savings} shows an example sustainability-beneficial action
