...
\captionsetup[subfigure]{justification=centering}
     ...
     \end{subfigure}
        ...
        mineral resource depletion\index{mineral resource depletion (MRD)} than the bicycle; see Figure~\ref{fig:auto_gwp} 
...
\item \label{problem:blog_social_media}
      Start a blog or social media account dedicated to local politics. 
