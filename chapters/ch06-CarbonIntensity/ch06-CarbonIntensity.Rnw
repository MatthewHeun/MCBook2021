...
iparc_df <- impact %>% 
  ...
  magrittr::set_names(c("Year", "$I$ [Gt/yr]", "$P$ [billions]", "$A$ [k\\$/person]", "$R$", "$C$"))
...
r_R <- iparc_df %>% trim_for_chapter(var = "$R$") %>% rate_func()
r_C <- iparc_df %>% trim_for_chapter(var = "$C$") %>% rate_func()
...
print(xtable::xtable(iparc_df, digits = c(0, 0, 3, 2, 2, 0, 0),
