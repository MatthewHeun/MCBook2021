...
%#############################################################
\chapter{Contamination intensity of resources ($C$)}
...
final term in Equation~\ref{eq:IPARC},
the environmental contamination intensity of resource extraction and consumption ($C$).
Resource extraction and consumption were discussed in Chapter~\ref{chap:resource_intensity}.
Environmental contamination is discussed in this chapter.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Contamination intensity of resource extraction}
\label{sec:contamination_intensity}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The contamination intensity of resource extraction and consumption ($C$)
represents the amount of environmental contamination (emitted wastes)
per unit of extracted and consumed resources (ingested materials).
Examples of environmental contamination include
...
specific examples of environmental contamination caused by resource extraction and consumption include:
%
\begin{itemize}

  \item Air, water, and soil pollution~\cite{Lee:2017ui} (environmental contamination) in the form of
        ...
        from bauxite mining (material extraction),

  \item Toxic tailings (environmental contamination)
        from tungsten\index{tungsten (W)} (material extraction)
        mining operations~\cite{Lee:2021tz}, and

  \item Deforestation\index{deforestation} and
        reduced forest quality~\cite{Kissinger:2020ut} (environmental contamination)
        to make way for rubber plantations\index{rubber plantations} (material extraction)
        in southeast Asia.
...
energy is the prime example of material extraction and consumption.
Because CO$_2$ emissions\index{carbon dioxide (CO$_2$)!emissions of}
are a significant source of environmental contamination
coming from energy consumption,
it follows that the prime example of environmental contamination
is CO$_2$ emissions.
Thus, the prime example for contamination intensity ($C$ in Equation~\ref{eq:IPARC})
is the CO$_2$ intensity of energy.
The units for the CO$_2$ intensity of energy are,
for example, tonnes of CO$_2$ emitted per MJ of energy consumed.

Figure~\ref{fig:LLNL_CO2_sankey} is a Sankey\index{Sankey diagram!CO$_2$ emissions} diagram of
sources of CO$_2$ emissions from the U.S.\ economy in 2018.
However, in Figure~\ref{fig:LLNL_CO2_sankey} CO$_2$ emissions
from electricity generation are not attributed to end uses for electricity.
Table~\ref{tab:corrected_co2_emissions} shows the result of attributing CO$_2$ emissions to end uses,
thereby completing the picture of U.S.\ carbon emissions.\index{carbon dioxide (CO$_2$)!emissions of}
Attributing CO$_2$ emissions from electricity generation
to end uses significantly increases the share of CO$_2$
emissions from the residential and commercial sectors.

\begin{figure}
\centering
\includegraphics[width=\figwidth]{chapters/ch06-CarbonIntensity/figures/Carbon_2018_United-States.png}
\caption{U.S.\ CO$_2$ emissions 2018~\cite{Lawrence-Livermore-National-Laboratory:wp}. 
...
US_CO2_by_sector <- readxl::read_excel(file.path("chapters", "ch06-CarbonIntensity", "datasets", "US-Carbon-Emissions.xlsx"),
                                       sheet = "data to plot") %>%
  dplyr::mutate(
    `CO2 share` = `CO2 share` * 100 # To put in % terms.
  ) %>% 
  dplyr::rename(
    `CO$_2$ emissions [Mt]` = `CO2 emissions allocated [tonnes]`,
...
as shown in Figure~\ref{fig:CarbonIntensityGraph}.
The line in Figure~\ref{fig:CarbonIntensityGraph}
...
already sequestered into the biomass as the plants grew to maturity.
Fossil fuels are not carbon neutral;
their combustion releases CO$_2$ that was sequestered millions of years ago.
When the world transitioned away from traditional biofuels toward fossil fuels,
the carbon intensity of energy rose.


<<CarbonIntensityGraph, fig.cap="Carbon intensity of energy, $C$ in the IPARC identity. The line represents the ratio of Figure~\\ref{fig:CarbonEmissionsGraph} and Figure~\\ref{fig:PrimaryEnergyGraph} in each year.">>=
...
Equation~\ref{eq:IPARC} shows that reducing any of the right-hand side terms to 0
will eliminate CO$_2$ emissions.
That is, if population ($P$), 
affluence ($A$),
energy intensity of the economy ($I_p$, the prime example for $R$)
\textbf{or} carbon intensity of energy (the prime example for $C$)
go to 0,
CO$_2$ emissions will be eliminated.
Few people want to reduce population ($P$) or affluence ($A$).
The primary energy intensity of the economy ($R$) is declining
...
regarding the prime example, energy and CO$_2$ emissions.
(Note that other sustainability issues are likely to remain!)

Figure~\ref{fig:CarbonIntensityGraph} shows that the carbon intensity of energy
...
the carbon intensity of energy ($C$) will be require unprecedented changes
...
There are at least four often-discussed options for
transitioning to a low carbon intensity energy supply:
(a) carbon capture and storage;
(b) carbon sequestration;
(c) biomass; and
(d) wind, water, and solar (WWS).
Each option has its own challenges to implementation and 
none has yet been implemented on large enough scale to reduce CO$_2$ emissions;
...
CO$_2$ emissions are captured at the smokestack of a fossil-fuel electricity generation plant
...
There are several challenges associated with carbon sequestration.
First, existing sequestration technology has not proved cost-effective.  
Petra Nova, the only CCS plant in the U.S.\, injected CO$_2$ for enhanced oil recovery.
In 2020, the plant was shut down due to low oil prices, 
demonstrating the interrelation of the economics among primary energy sources.

Second, a fossil-fuel power plant fitted with carbon sequestration technology 
must ensure that the CO$_2$ emissions remains buried. 
If the CO$_2$ escapes to the atmosphere, the sequestration effort is ineffective.
...
storing it in, presumably, inert form in perpetuity.
...
directly, typically using huge banks of fans and a absorbent.
...
to the energy mix of the first half of the 18$^\mathrm{th}$ century,
...
to grow the required amount of crops for biomass purposes.
...
with expensive offerings from companies such as Telsa.
...
Nuclear power (again, an electricity source only) may provide a bridge to a sustainable future.
Nuclear power emits little CO$_2$ during plant operation. 
However, the energy cost to construct a nuclear power plant is higher than other types of
electricity production plants.
In addition, 
it is energy-intensive to mine and refine uranium prior to its use in a nuclear power plant. 
Again, similar to the buildout of renewables, if the energy required to construct nuclear energy plants
and refine uranium comes from fossil fuels, 
significant CO$_2$ emissions will result from the conversion to nuclear electricity.

One additional consideration is the availability of unranium resources,
the topic of an end-of-chapter project.
...
Carbon offsets are a way to enable carbon emitters to
pay someone else to sequester or reduce their own carbon emissions.
Carbon emissions are traded as a financial instrument.
A market develops in which emissions and reductions are traded, 
thereby finding a low price for net emission reductions.
As such, carbon offsets enable carbon emitters to shift the responsibility for implementing
emissions reductions via any of the techniques discussed above.
...
%++++++++++++++++++++++++++++++
Figure \ref{fig:CarbonIntensityGraph} shows that a transition to a new form(s)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Revisiting the IPARC identity}
\label{sec:revisit_IPARC}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Chapters~\ref{chap:planetary_boundaries}--\ref{chap:carbon_intensity}
show the evolution of all terms in the IPARC identity (Equation~\ref{eq:IPARC})
...
in the right-side terms ($P$, $A$, $R$, and $C$). 
To do so, we introduce a rate version of the IPARC equation.

In the rate version of the IPARC equation, each term is represented 
by an exponential rate of change ($r$), measured in \%/year.
The rate version of the IPARC equation indicates 
which terms are moving in a sustainable direction ($r < 0$) and
which terms are moving in an unsustainable direction ($r > 0$).

\begin{equation} \label{eq:IPARC_rate}
  r_I \approx r_P + r_A + r_R + r_C
\end{equation}
%
Each rate ($r$) is calculated by

\begin{equation} \label{eq:rate_calcs}
  r_y = \left[ \left( \frac{y_2}{y_1} \right) ^ \frac{1}{t_2 - t_1} - 1 \right] \times 100 \; ,
\end{equation}
%
where
$r$ is the rate of change in \%/year, 
$y$ is one of $I$, $P$, $A$, $R$, or $C$, 
...
rate_to_string <- function(rate) {
  paste(round(rate*100, 1), " \\%/yr")
}
...
resource_intensity <- energy_intensity %>%
  dplyr::select(Year, energy_per_GDP_MJ_per_dollar)
...
iparc_df <- impact %>% 
  ...
  magrittr::set_names(c("Year", "$I$ [Gt/yr]", "$P$ [billions]", "$A$ [k\\$/person]", 
                        "$R$ [MJ/\\$]", "$C$ [Mt/EJ]"))

# Extract the data to be used in the equation
r_I <- iparc_df %>% trim_for_chapter(var = "$I$ [Gt/yr]") %>% rate_func()
r_P <- iparc_df %>% trim_for_chapter(var = "$P$ [billions]") %>% rate_func()
r_A <- iparc_df %>% trim_for_chapter(var = "$A$ [k\\$/person]") %>% rate_func()
r_R <- iparc_df %>% trim_for_chapter(var = "$R$ [MJ/\\$]") %>% rate_func()
r_C <- iparc_df %>% trim_for_chapter(var = "$C$ [Mt/EJ]") %>% rate_func()

# r_I_check should be same as r_I
r_I_check <- r_P + r_A + r_R + r_C

stopifnot(abs(r_I_check - r_I) < 0.0005)
@
...
and~\ref{fig:CarbonIntensityGraph} 
are shown in Table~\ref{tab:iparc_select_years}.
Data from Table~\ref{tab:iparc_select_years}
can be used with Equation~\ref{eq:rate_calcs}
to answer the question, 
``what equivalent annual rate of change ($r$) 
would give the same start and end points
in 1800 and 2019?''
The answers are shown in Equation~\ref{eq:IPARC_rate_with_numbers}.


<<IPARC_Table, results="asis">>=
years_for_rate_table <- c(1800, 1850, 1900, 1950, 1970, 2000, 2010, 2019)

print(xtable::xtable(iparc_df %>% 
                       dplyr::filter(Year %in% years_for_rate_table), digits = c(0, 0, 3, 2, 2, 2, 2),
                     caption = "IPARC terms from selected years in Figures~\\ref{fig:CarbonEmissionsGraph}, \\ref{fig:PopulationGraph}, \\ref{fig:AffluenceGraph-Nordhaus}, \\ref{fig:EnergyIntensityGraph-Nordhaus}, and~\\ref{fig:CarbonIntensityGraph}.",
                     label = "tab:iparc_select_years"),
      include.rownames = FALSE,
      include.colnames = TRUE,
      caption.placement = "top",
      type = "latex",
...
\begin{equation}
\label{eq:IPARC_rate_with_numbers}
\small
\begin{tabular}{ccccccccc}
$r_I$                       & $\approx$ & $r_P$                       & $+$ & $r_A$                       & 
                                    $+$ & $r_R$                       & $+$ & $r_C$                         \\
\Sexpr{rate_to_string(r_I)} & $\approx$ & \Sexpr{rate_to_string(r_P)} & $+$ & \Sexpr{rate_to_string(r_A)} &
                                    $-$ & \Sexpr{rate_to_string(abs(r_R))} & $+$ & \Sexpr{rate_to_string(r_C)}   \\
\end{tabular}
\normalsize
\end{equation}

Equation~\ref{eq:IPARC_rate_with_numbers} shows that carbon emissions 
(Figure~\ref{fig:CarbonEmissionsGraph})
are increasing 
at an average rate of $r_I =$ \Sexpr{rate_to_string(r_I)} from 1800--2019. 
The declining energy intensity of the economy 
is the only term in the IPARC identity
putting downward pressure on CO$_2$ emissions ($r_R < 0$).
All other terms on the right side of Equation~\ref{eq:IPARC_rate_with_numbers}
are greater than zero and driving CO$_2$ emissions upward.
The positive terms represent the sources of our sustainability challenges
for the prime example:
rising population ($r_P > 0$), 
rising affluence ($r_A > 0$), and 
expanded use of fossil fuels ($r_C > 0$).
Together with Equation~\ref{eq:IPARC_rate_with_numbers},
the data in Table~\ref{tab:iparc_select_years} can be used to discern
trends over shorter periods of time. 

The IPARC identity and 
the prime example (energy consumption and CO$_2$ emissions)
show that we are heading in an unsustainable direction,
as CO$_2$ emissions are accelerating over time.
In other words,
Part~\ref{part:IPARC} (Chapters~\ref{chap:planetary_boundaries}--\ref{chap:carbon_intensity})
has described the \emph{what} of our sustainability challenges.
Part~\ref{part:sustainability_challenges}
...
The final part of this book (Part~\ref{part:actions},
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{IPARC interactions}
\label{sec:iparc_interactions}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The IPARC identity
(especially applied to the prime example of energy and CO$_2$ emissions)
raises questions of interactions among its terms.
...
the carbon intensity of energy ($C$ in the IPARC identity) will go down.
That said, energy efficiency is known to stimulate economic activity 
by a mechanism called the ``rebound effect,''
wherein monetary savings (from reduced energy needs)
are reinvested, causing economic growth.
The economic growth raises the GDP of an economy,
so $A$ will increase.
...
The rebound effect is one example of interactions among the terms of the IPARC identity.
Policies directed at one term of the IPARC identity
(in this example, the carbon intensity of energy, $C$)
will have effects on other terms of the IPARC identity
(in this example, affluence, $A$).
...
Energy Vault simulation: \url{https://vimeo.com/335818817}.
...
\item Do renewable energy sources have ``emissions?'' If so, what are they?
        Make reference to Equation \ref{eq:IPARC}.

\item Fossil fuels have impacts other than CO$_2$. One example is oil spills,
      such as the Exxon Valdez and the Deep Water Horizon disaster. (The 
       International Tanker Owners Pollution Federation reports ``only'' 18 oil
       spills larger than 700 tonnes for the period 2010-2019 versus 245 in the 
       1970s.) Other examples include release or contamination hydraulic fracking
       fluid; air quality issues associated with burning fossil fuels such as
       soot, smog, and ozone; and soil contamination from leaking underground 
       fuel storage tanks. The tendency in a technocratic society is to approach
       these problems as ``separate'' problems and to propose technological fixes
       specific to each problem. Indeed, the rate of oil spills has gone down due
       to better technology. How do we manage tradeoffs among different types of
       \emph{solutions}? Technological solutions come with their own costs, risks, 
       and inherent tradeoffs. Give some examples of the tradeoffs inherent in 
       pollution prevention technologies.
       
\item Expanding on the issues raised in the previous question, 
      ...
      How do you make decisions about trading off the different types of 
      environmental impacts?
      
\item The naive tendency is to read the previous two questions and just say that
      all impacts are bad, and that is true by definition of the word ``impact.''
      But it is not helpful. The more interesting and helpful question is the 
      debate between a cost/benefit analysis (do the benefits provided by this
      form of energy outweigh the costs?) and the question ``is this form of 
      energy sustainable and, if so, for how long?'' Do you lean more to the 
      cost/benefit side or the sustainability side and why? Are you using a weak
      or strong sustainability criterion?
      
\item Some people object to wind turbine because they kill birds. 
      All human structures kill birds. 
      ...
      Whether there will be a cap on the total number of wind turbines is probably
      the same as whether there will be a cap on the extent of human civilization.
      Does the whole Earth belong to humans or do animals get a place ``of their own?''

\item Which would you prefer, a biomass energy system or a WWS energy system? Why?

\item What would be the consequences of a reduction in $EROI$ that would accompany a conversion to 
        a biofuel-based energy system?
        
\item we have to have storage TO we'd like to have storage. Is it really required?

\item Consider the energy transition shown in Figure \ref{fig:CarbonIntensityGraph}.
      What does Figure \ref{fig:CarbonIntensityGraph} indicate about the length
...
\item Should the transition to renewable energy sources be actively managed or should 
      we let economics do it for us? 
...
\item The big issue for carbon capture and storage/sequestration is who are the 
...
\begin{projects}
  ...
  \item Research the potential for CO$_2$ storage in graphene. 
        ...
        How many additional solar panels 
  ...
  \item Derive Equation~\ref{eq:IPARC_rate} from Equation~\ref{eq:IPARC}.
        You will use $(1 + r_1)(1 + r_2) \approx r_1 + r_2 + 1$ for $r_1, r_2 <\!\!\!< 1$
        in the derivation.
  
  \item Verify the calculations in Equation~\ref{eq:IPARC_rate_with_numbers}
        using data from Table~\ref{tab:iparc_select_years}.
        Then recalculate the terms in Equation~\ref{eq:IPARC_rate_with_numbers} 
        ...
        of Equation~\ref{eq:IPARC_rate_with_numbers}?
