...
rate_to_string <- function(rate, include_units = TRUE) {
  if (include_units) {
    return(paste(format(round(rate*100, 1), nsmall = 1), "\\%/yr"))
  }
...
are increasing 
at an average rate of $r_I =$ \Sexpr{rate_to_string(r_I_220)}
since pre-industrial times (1800--2019)
and $r_I =$ \Sexpr{rate_to_string(r_I_20)}
over the last two decades (2000--2019). 
(The data in Table~\ref{tab:iparc_select_years} and 
Equation~\ref{eq:IPARC_rate_with_numbers}
can be used to discern
trends over other periods of time. 
See Project~P6.\ref{item:iparx_calcs}.)
The declining energy intensity of the economy term ($r_R$ < 0)
is the only term in the IPARX identity
putting downward pressure on the CO$_2$ emissions growth rate ($r_I$).
...
Interestingly, the impact intensity of energy ($r_X$)
is smaller (\Sexpr{rate_to_string(r_C_20)})
in the past two decades than over the past two centuries
(\Sexpr{rate_to_string(r_C_220)}), because 
the world's transition to a fully carbonized energy system
was largely complete by 1970.
(See Figure~\ref{fig:CarbonIntensityGraph}.)
The effect of the declining resource intensity of the economy ($r_R < 0$)
is growing over time 
($r_R = -$\Sexpr{rate_to_string(abs(r_R_220))} for 1800--2019 and
$r_R = -$\Sexpr{rate_to_string(abs(r_R_20))} for 2000--2019).

%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
\begin{mcframe}[0.90\textwidth](0.85\textwidth)
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The positive terms in Equation~\ref{eq:IPARC_rate_with_numbers}
represent sources of our sustainability challenges:
rising population ($r_P$ > 0), 
rising affluence ($r_A$ > 0), and 
expanded use of fossil fuels ($r_X$ > 0).
...
will have effects on other terms of the IPARX identity.
...
the $R$ or $X$ terms in the IPARX identity.
...
\begin{projects}
  ...
  \item Verify the calculations in Equation~\ref{eq:IPARC_rate_with_numbers}
        ...
        of Equation~\ref{eq:IPARC_rate_with_numbers}? \label{item:iparx_calcs}
