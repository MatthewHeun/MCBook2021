...
%#############################################################
\chapter{Contamination intensity of resources ($X$)}
...
final term in Equation~\ref{eq:IPARX},
the environmental \emph{impact intensity} of resource extraction and consumption ($X$).
$X$ is a function of $R$ (resource extraction and consumption) discussed in 
...
$X$ represents the economy's excreted wastes.
...
(If you are comfortable with matrix notation, $X$ can be though of as a matrix with
...
The impact intensity of resource extraction and consumption ($X$)
...
($X$ in Equation~\ref{eq:IPARX}) is CO$_2$ emissions.
Specifically, the prime example for impact intensity ($X$)
...
\emph{or} carbon intensity of energy $I_{CO_2}$ (the prime example for $X$)
go to zero,
...
in the right-side terms ($P$, $A$, $R$, and $X$). 
...
r_I_220 <- iparx_df %>% trim_for_chapter(var = "$I$ [Gt/yr]") %>% rate_func()
r_P_220 <- iparx_df %>% trim_for_chapter(var = "$P$ [billions]") %>% rate_func()
r_A_220 <- iparx_df %>% trim_for_chapter(var = "$A$ [k\\$/person]") %>% rate_func()
r_R_220 <- iparx_df %>% trim_for_chapter(var = "$R$ [MJ/\\$]") %>% rate_func()
r_X_220 <- iparx_df %>% trim_for_chapter(var = "$X$ [Mt/EJ]") %>% rate_func()

r_I_20 <- iparx_df %>% trim_for_chapter(var = "$I$ [Gt/yr]", years = c(2000, 2019)) %>% rate_func()
r_P_20 <- iparx_df %>% trim_for_chapter(var = "$P$ [billions]", years = c(2000, 2019)) %>% rate_func()
r_A_20 <- iparx_df %>% trim_for_chapter(var = "$A$ [k\\$/person]", years = c(2000, 2019)) %>% rate_func()
r_R_20 <- iparx_df %>% trim_for_chapter(var = "$R$ [MJ/\\$]", years = c(2000, 2019)) %>% rate_func()
r_X_20 <- iparx_df %>% trim_for_chapter(var = "$X$ [Mt/EJ]", years = c(2000, 2019)) %>% rate_func()
...
print(xtable::xtable(iparx_df %>% 
                     ...
                     caption = "IPARX terms from selected years in Figures~\\ref{fig:CarbonEmissionsGraph}, \\ref{fig:PopulationGraph}, \\ref{fig:AffluenceGraph-Nordhaus}, \\ref{fig:EnergyIntensityGraph-Nordhaus}, and~\\ref{fig:CarbonIntensityGraph}.",
...
In other words,
Part~\ref{part:IPARX} (Chapters~\ref{chap:planetary_boundaries}--\ref{chap:carbon_intensity})
...
% the carbon intensity of energy ($X$ in the IPARX identity) will go down.
