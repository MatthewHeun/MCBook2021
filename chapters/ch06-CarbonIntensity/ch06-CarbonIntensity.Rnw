...
the \emph{environmental \textbf{contamination intensity} of resource extraction and consumption} ($C$).
$C$ is a function of $R$ (resource extraction and consumption) discussed in 
Chapter~\ref{chap:resource_intensity} in that the 
type and intensity of contamination is directly related to the types, quantities, 
and processing of resources. 
%Environmental contamination is discussed in this chapter.
In some sense, $R$ represents the inputs to the economy 
(``food'' in the metabolic analogy) and 
$C$ represents the economy's excreted wastes.
...
\begin{equation}\label{eq:R}
  C = \frac{\textrm{impacts produced}}{\textrm{resources extracted to make \$1 GDP}} \; .
\end{equation}
Note that ``impacts'' are plural.
Extracting and consuming all the resources necessary to make a dollar of GDP will have effects
on air, water, species diversity, land, and human communities.
(If you are comfortable with matrix notation, $C$ can be though of as a matrix with
...
it follows that CO$_2$ emissions are the prime example of 
environmental contamination intensity of resource extraction and consumption.
Specifically, the prime example for contamination intensity ($C$ in Equation~\ref{eq:IPARC})
...
\item Do renewable energy sources have ``emissions?'' If so, what are they?
        Make reference to Equation \ref{eq:IPARC}. At a minimum, discuss
...
\item Fossil fuels have environmental impacts other than CO$_2$. 
      ...
      Other examples include contamination by hydraulic fracking
