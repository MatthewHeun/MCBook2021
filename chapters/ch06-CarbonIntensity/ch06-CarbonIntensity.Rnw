...
over the period 1800--today
for the prime example of energy and CO$_2$ emissions.
It is instructive to see how the righ-side terms ($P$, $A$, $R$, and $C$) 
...
which terms are headed in a sustainable direction ($r < 0$) and
which terms are headed in an unsustainable direction ($r > 0$).
...
and~\ref{fig:CarbonIntensityGraph} can be evaluated
to answer the question, 
``what equivalent annual rate of change ($r$) 
would give the same starting and end points
in 1800 and 2018?''
The answer is shown in Equation~\ref{eq:IPARC_rate_with_numbers}
...
% $r_I$       & $=$ & $r_P$       & $+$ & $r_A$       & $+$ & $r_R$       & $+$ & $r_C$ 
% \Sexpr{r_I} & = & \Sexpr{r_P} & + & \Sexpr{r_A} & + & \Sexpr{r_R} & + & \Sexpr{R_C} \\
...
\begin{questions}
  ...
  \item Should we manage our energy transition (to renewables) or should we let economics do it f
        or us? Do we have time to wait? 
