...
the environmental \emph{contamination intensity} of resource extraction and consumption ($C$).
$C$ is a function of $R$ (resource extraction and consumption) discussed in 
Chapter~\ref{chap:resource_intensity} in that the 
type and intensity of contamination is directly related to 
the types, quantities, and processing of resources. 
%Environmental contamination is discussed in this chapter.
$R$ represents the inputs to the economy 
...
\begin{equation}\label{eq:C}
  C = \frac{\textrm{total impacts produced}}{\textrm{total resources extracted}} \; .
\end{equation}
%
...
($C$ in Equation~\ref{eq:IPARC}) is CO$_2$ emissions.
Specifically, the prime example for contamination intensity ($C$)
is the CO$_2$ intensity of energy, $I_{CO_2}$.
The units for $I_{CO_2}$ are,
...
\begin{figure}
\centering
\includegraphics[width=\linewidth]{chapters/ch06-CarbonIntensity/figures/Carbon_2018_United-States.png}
...
<<CarbonIntensityGraph, fig.cap="Carbon intensity of energy, $I_{CO_2}$. The line represents the ratio of Figure~\\ref{fig:CarbonEmissionsGraph} and Figure~\\ref{fig:PrimaryEnergyGraph} in each year.">>=
...
affluence ($A$),
energy intensity of the economy ($I_{ep}$, the prime example for $R$)
\emph{or} carbon intensity of energy $I_{CO_2}$ (the prime example for $C$)
go to zero,
CO$_2$ emissions will be eliminated.
Few people want to reduce population ($P$) or affluence ($A$).
The primary energy intensity of the economy, $I_{ep}$, is declining
...
the carbon intensity of energy ($I_{CO_2}$) will require unprecedented changes
...
directly, typically using huge banks of fans and a adsorbent.
...
\noindent \textbf{What about nuclear power?}
...
\noindent \textbf{What about offsets?}
...
r_I_220 <- iparc_df %>% trim_for_chapter(var = "$I$ [Gt/yr]") %>% rate_func()
r_P_220 <- iparc_df %>% trim_for_chapter(var = "$P$ [billions]") %>% rate_func()
r_A_220 <- iparc_df %>% trim_for_chapter(var = "$A$ [k\\$/person]") %>% rate_func()
r_R_220 <- iparc_df %>% trim_for_chapter(var = "$R$ [MJ/\\$]") %>% rate_func()
r_C_220 <- iparc_df %>% trim_for_chapter(var = "$C$ [Mt/EJ]") %>% rate_func()

r_I_20 <- iparc_df %>% trim_for_chapter(var = "$I$ [Gt/yr]", years = c(2000, 2019)) %>% rate_func()
r_P_20 <- iparc_df %>% trim_for_chapter(var = "$P$ [billions]", years = c(2000, 2019)) %>% rate_func()
r_A_20 <- iparc_df %>% trim_for_chapter(var = "$A$ [k\\$/person]", years = c(2000, 2019)) %>% rate_func()
r_R_20 <- iparc_df %>% trim_for_chapter(var = "$R$ [MJ/\\$]", years = c(2000, 2019)) %>% rate_func()
r_C_20 <- iparc_df %>% trim_for_chapter(var = "$C$ [Mt/EJ]", years = c(2000, 2019)) %>% rate_func()



# r_I_check should be same as r_I
r_I_check_220 <- r_P_220 + r_A_220 + r_R_220 + r_C_220
r_I_check_20 <- r_P_20 + r_A_20 + r_R_20 + r_C_20
...
are increasing 
at an average rate of $r_I =$ \Sexpr{rate_to_string(r_I_220)} from 1800--2019. 
...
because CO$_2$ emissions are accelerating over time.
In other words,
...
The thinking goes that either the resource intensity ($R$ in the IPARX identity, 
Equation~\ref{eq:iparc}) 
or impact intensity ($X$), or both,
...
so affluence ($A$) will increase.
As $A$ increases, energy consumption will increase.
...
``backfire'' is said to occur~\cite{Sorrell:2010ul}~\cite{Sorrell:2009cg}.
...
the $R$ or $C$ terms in the IPARC identity.
We may inadvertently affect $A$ to the effect of undoing 
...
I.e., an increase in $A$ leads to a decrease in the rate of growth of $P$.
...
Historically, as GDP (equal to $P \times A$) increases, primary energy also rises,
...
egdp_df %>%
  ...
  ggplot2::labs(x = "GDP [trillion 2017 international dollars]", 
...
Figure~\ref{fig:E_vs_GDP_graph} shows that
each additional unit of GDP requires incrementally less energy than the one before.
The line in Figure~\ref{fig:E_vs_GDP_graph} is concave downward because
the primary energy intensity of the economy ($I_{ep}$)
has been decreasing over time,
as shown in Figure~\ref{fig:EnergyIntensityGraph-Nordhaus}.
Thus, as GPD ($P \times A$) rises, 
the primary energy intensity of the economy
($R$ in the IPARX equation for the prime example) 
is expected to fall, 
as it has been for 220 years.
(See Figure~\ref{fig:EnergyIntensityGraph-Nordhaus}.)
...
will stimulate enconomic growth and increase affluence.
If they are right, 
a renewables-driven decrease in $X$ would lead to an increase in $A$
in the IPARX equation (Equation~\ref{eq:IPARC}).
...
environmental impacts ($I$) or CO$_2$ emissions (in the prime example).
...
Edward Humes, \emph{Garbology: Our Dirty Love Affair with Trash}, Avery, 2012.
