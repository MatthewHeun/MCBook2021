...
to become more sustainable despite 
...
will prevent additional carbon from being embodied\index{embodied carbon} in a new vehicle.
...
Individuals can choose to live in cities, which, on a per-person basis
...
Typically, climate-friendly diets involve reductions of meat consumption, particularly beef\index{beef}.
...
\item The line between individual\index{individuals} and collective action\index{collective action}
      ...
      Are organizing and attending the rally on different sides of the line?
...
\item \label{problem:more_actions} 
      Figure~\ref{fig:CO2-Savings} shows an example of a sustainability-beneficial action\index{sustainability action}
