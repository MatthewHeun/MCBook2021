...
$\left[\frac{\textrm{impact}}{\textrm{year}}\right]$ & & $\left[\mathrm{persons}\right]$ &  & $\left[\frac{\textrm{GDP}}{\textrm{person$\cdot$year}}\right]$ &  & $\left[\frac{\textrm{resources}}{\textrm{GDP}}\right]$ &  & $\left[\frac{\textrm{impact}}{\textrm{resources}}\right]$
\end{tabular}
\normalsize
}
...
library(ggsankey)          # For ggplot-style Sankey diagrams. Install with devtools::install_github("davidsjoberg/ggsankey")
...
library(webshot)           # Required by networkD3
...
# These color definitions come from the file morgan2.sty.
mc_title_blue <- "#0d5e92"
data_line_size <- 1
data_line_colour <- mc_title_blue
data_linetype <- "solid"
curve_fit_size <- 0.5
curve_fit_colour <- "red"
curve_fit_linetype <- "dashed"
guide_line_size <- 0.2
...
%#############################################################
\part{Towards Sustainability: Collective and Personal Actions}
\label{part:actions}
%#############################################################
